...
public class LoginAttemptTracker {
  ...
  public LoginAttemptTracker(
      StringRedisTemplate stringRedisTemplate, AuthSecurityProperties securityProperties) {
  ...
  public void assertLoginAllowed(String tenantCode, String username, String clientIp) {
    ...
    if (locked != null) {
      log.warn(
          "security_event event=login_locked tenantCode={} username={} clientIp={}",
          tenantCode,
          username,
          clientIp);
      throw new com.sunny.datapillar.common.exception.TooManyRequestsException(
  ...
  public void recordFailure(String tenantCode, String username, String clientIp) {
    ...
    if (count != null && count == 1L) {
      stringRedisTemplate.expire(
          failKey, Duration.ofSeconds(securityProperties.getLogin().getWindowSeconds()));
    }
    if (count != null && count >= securityProperties.getLogin().getMaxAttempts()) {
      String lockKey = buildLockKey(tenantCode, username, clientIp);
      stringRedisTemplate
          .opsForValue()
          .set(lockKey, "1", Duration.ofSeconds(securityProperties.getLogin().getLockSeconds()));
      log.warn(
          "security_event event=login_lock_applied tenantCode={} username={} clientIp={} failCount={}",
          tenantCode,
          username,
          clientIp,
          count);
