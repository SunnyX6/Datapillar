...
/**
 * Store for session-state lifecycle management.
 *
 * @author Sunny
 * @date 2026-01-01
 */
...
public class SessionStateStore {
  ...
  private static final String ROTATE_SESSION_SCRIPT =
  ...
  public void openSession(
    ...
    stringRedisTemplate
        .opsForValue()
        .set(SessionStateKeys.sessionStatusKey(sid), SessionStateKeys.STATUS_ACTIVE, sessionTtl);
    stringRedisTemplate
        .opsForValue()
        .set(SessionStateKeys.sessionTenantKey(sid), String.valueOf(tenantId), sessionTtl);
    stringRedisTemplate
        .opsForValue()
        .set(SessionStateKeys.sessionUserKey(sid), String.valueOf(userId), sessionTtl);
    stringRedisTemplate
        .opsForValue()
        .set(SessionStateKeys.sessionAccessJtiKey(sid), accessJti, sessionTtl);
    stringRedisTemplate
        .opsForValue()
        .set(SessionStateKeys.sessionRefreshJtiKey(sid), refreshJti, sessionTtl);
  ...
  public boolean isAccessTokenActive(String sid, String accessJti) {
    ...
    String tokenStatus =
        stringRedisTemplate.opsForValue().get(SessionStateKeys.tokenStatusKey(accessJti));
    if (!SessionStateKeys.STATUS_ACTIVE.equals(tokenStatus)) {
      return false;
    }
    String tokenSid =
        stringRedisTemplate.opsForValue().get(SessionStateKeys.tokenSessionKey(accessJti));
  ...
  public void revokeSession(String sid) {
    ...
    String currentAccessJti =
        stringRedisTemplate.opsForValue().get(SessionStateKeys.sessionAccessJtiKey(sid));
    long ttlSeconds = resolveRemainingTtlSeconds(statusKey);
    stringRedisTemplate
        .opsForValue()
        .set(statusKey, SessionStateKeys.STATUS_REVOKED, Duration.ofSeconds(ttlSeconds));
  ...
  public boolean replaceAccessToken(
    ...
    if (currentAccessJti != null
        && !currentAccessJti.isBlank()
        && !currentAccessJti.equals(newAccessJti)) {
      revokeAccessToken(currentAccessJti);
    }
    return true;
  }
  ...
  public void revokeAccessToken(String accessJti) {
    ...
    stringRedisTemplate
        .opsForValue()
        .set(tokenStatusKey, SessionStateKeys.STATUS_REVOKED, Duration.ofSeconds(ttlSeconds));
  }
  ...
  private void activateAccessToken(String sid, String accessJti, long accessTtlSeconds) {
    ...
    stringRedisTemplate
        .opsForValue()
        .set(SessionStateKeys.tokenStatusKey(accessJti), SessionStateKeys.STATUS_ACTIVE, accessTtl);
    stringRedisTemplate
        .opsForValue()
        .set(SessionStateKeys.tokenSessionKey(accessJti), sid, accessTtl);
  }
