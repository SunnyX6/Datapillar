...
/**
 * Cookie manager for auth-related cookies.
 *
 * @author Sunny
 * @date 2026-01-01
 */
...
public class AuthCookieManager {
  ...
  public void setAuthCookies(
      HttpServletResponse response, String accessToken, String refreshToken, Boolean rememberMe) {
    int accessMaxAge = Math.toIntExact(jwtToken.getAccessTokenExpiration());
    int refreshMaxAge =
        Math.toIntExact(jwtToken.getRefreshTokenExpiration(Boolean.TRUE.equals(rememberMe)));

    ResponseCookie accessTokenCookie =
        ResponseCookie.from("auth-token", accessToken)
            .httpOnly(true)
            .secure(cookieSecure)
            .path("/")
            .maxAge(accessMaxAge)
  ...
  public void setLoginTokenCookie(
      HttpServletResponse response, String loginToken, long ttlSeconds) {
    ResponseCookie loginTokenCookie =
        ResponseCookie.from("login-token", loginToken)
            .httpOnly(true)
            .secure(cookieSecure)
            .path("/login")
            .maxAge(ttlSeconds)
  ...
  public void issueBusinessCsrfCookie(
      Long tenantId, Long userId, long ttlSeconds, HttpServletResponse response) {
  ...
  public void issueSessionCsrfCookies(
      Long tenantId, Long userId, long refreshTtlSeconds, HttpServletResponse response) {
  ...
  public void clearAuthCookies(HttpServletResponse response) {
    ...
    ResponseCookie refreshCsrfCookie =
        ResponseCookie.from(refreshCsrfCookieName, "")
            .httpOnly(false)
            .secure(cookieSecure)
            .path("/")
            .maxAge(0)
  ...
  public void clearLoginTokenCookie(HttpServletResponse response) {
    ResponseCookie loginTokenCookie =
        ResponseCookie.from("login-token", "")
            .httpOnly(true)
            .secure(cookieSecure)
            .path("/login")
            .maxAge(0)
  ...
  private void issueRefreshCsrfCookie(
      Long tenantId, Long userId, long ttlSeconds, HttpServletResponse response) {
    if (!securityProperties.getCsrf().isEnabled()) {
      return;
    }
