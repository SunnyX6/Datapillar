...
class AuthServiceImplTest {
  ...
  void setUp() {
    ...
    properties.validate();
    jwtToken = new JwtToken(properties, new KeySetKeyManager(new ObjectMapper(), properties));
    tokenEngine = new JwtTokenEngine(jwtToken, new ClaimAssembler());
  }

  @Test
  void refreshToken_shouldKeepRotatedJwtAndSessionStateJtiAligned() {
    ...
    when(sessionStore.isSessionActive("sid-1")).thenReturn(true);
    when(sessionStore.rotateForRefresh(any(), any(), any(), any(), eq(120L), eq(3600L)))
        .thenReturn(new SessionStore.RotateResult(true, false, false, "access-old"));
    ...
    ArgumentCaptor<String> presentedRefreshJtiCaptor = ArgumentCaptor.forClass(String.class);
    ArgumentCaptor<String> newRefreshJtiCaptor = ArgumentCaptor.forClass(String.class);
    ArgumentCaptor<String> newAccessJtiCaptor = ArgumentCaptor.forClass(String.class);
    verify(sessionStore)
        .rotateForRefresh(
            sidCaptor.capture(),
            presentedRefreshJtiCaptor.capture(),
            newRefreshJtiCaptor.capture(),
            newAccessJtiCaptor.capture(),
            eq(120L),
            eq(3600L));

    assertEquals("sid-1", sidCaptor.getValue());
    assertEquals("refresh-old", presentedRefreshJtiCaptor.getValue());
    ...
    assertEquals("sid-1", accessClaims.get("sid", String.class));
