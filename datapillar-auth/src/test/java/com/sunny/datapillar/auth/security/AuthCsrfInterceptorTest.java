...
class AuthCsrfInterceptorTest {
  ...
  void preHandle_refreshShouldFallbackToRefreshTokenWhenAccessExpired() {
    ...
    AuthCsrfInterceptor interceptor = new AuthCsrfInterceptor(properties, csrfTokenStore, jwtToken);

    MockHttpServletRequest request = new MockHttpServletRequest();
    request.setMethod("POST");
    request.setRequestURI("/auth/session/refresh");
    ...
    Claims refreshClaims = Jwts.claims().setSubject("1").build();
    when(jwtToken.parseToken("valid-refresh-token")).thenReturn(refreshClaims);
    when(jwtToken.getTenantId(refreshClaims)).thenReturn(10L);
    when(jwtToken.getUserId(refreshClaims)).thenReturn(1L);
  ...
  void preHandle_nonRefreshShouldFallbackToRefreshTokenWhenAccessExpired() {
    ...
    MockHttpServletRequest request = new MockHttpServletRequest();
    request.setMethod("POST");
    request.setRequestURI("/auth/session/revoke");
    request.addHeader("Origin", "http://localhost:3001");
    ...
    Claims refreshClaims = Jwts.claims().setSubject("1").build();
    when(jwtToken.parseToken("expired-access-token"))
        .thenThrow(
            new com.sunny.datapillar.common.exception.UnauthorizedException("Token has expired"));
    when(jwtToken.parseToken("valid-refresh-token")).thenReturn(refreshClaims);
    when(jwtToken.getTenantId(refreshClaims)).thenReturn(10L);
    when(jwtToken.getUserId(refreshClaims)).thenReturn(1L);
