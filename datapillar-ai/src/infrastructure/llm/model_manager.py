...
- 这里负责“读取 + 选择默认模型”的逻辑；纯数据访问由 `ModelRepository` 负责。
"""
...
from src.infrastructure.repository.system.repository import ModelRepository
...
class ModelManager:
    ...
    def default_chat_model(self) -> ModelConfig | None:
        """获取默认 Chat 模型"""
        row = ModelRepository.get_chat_default()
        if row:
    ...
    def default_embed_model(self) -> ModelConfig | None:
        """获取默认 Embedding 模型"""
        row = ModelRepository.get_embedding_default()
        if row:
            return ModelConfig(row)
