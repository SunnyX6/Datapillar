...
class JobRepository:
    ...
    @staticmethod
    def get(job_id: int, user_id: int) -> dict[str, Any] | None:
        query = text(
            """
            SELECT
              job_id,
            ...
            WHERE job_id = :job_id AND created_by = :created_by
            """
        )
        with MySQLClient.get_engine().connect() as conn:
            row = conn.execute(query, {"job_id": job_id, "created_by": user_id}).mappings().fetchone()
        ...
        *,
        ...
        query = text(
            ...
            SELECT
              ...
              updated_at
            FROM knowledge_document_job
            WHERE document_id = :document_id AND created_by = :created_by
        ...
        params = {"document_id": document_id, "created_by": user_id, "limit": limit, "offset": offset}
        with MySQLClient.get_engine().connect() as conn:
            rows = [dict(row) for row in conn.execute(query, params).mappings()]
            total = conn.execute(
                count_query, {"document_id": document_id, "created_by": user_id}
            ).scalar_one()
    ...
    def mark_error(job_id: int, message: str) -> None:
        ...
        with MySQLClient.get_engine().begin() as conn:
            conn.execute(query, {"job_id": job_id, "message": message})
