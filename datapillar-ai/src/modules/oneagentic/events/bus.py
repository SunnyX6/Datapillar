...
class EventBus:
    ...
    使用示例：
    ```python
    from src.modules.oneagentic.events import event_bus, AgentStartedEvent
    ...
    # 异步处理器
    @event_bus.on(AgentStartedEvent)
    async def on_agent_started_async(source, event):
