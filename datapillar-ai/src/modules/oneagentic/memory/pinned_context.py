...
class PinnedContext(BaseModel):
    ...
    def to_prompt(self) -> str:
        ...
        ### 当前工作
        [AgentTodoList.to_prompt()]

        ### 已有产物
        - [type] summary (ref: xxx)
        """
        if self.is_empty():
            return ""
        ...
        if self.decisions:
            lines.append("")
            lines.append("### 关键决策")
            for d in self.decisions[-10:]:  # 最多显示 10 条
        ...
        if self.constraints:
            lines.append("")
            lines.append("### 用户约束")
            for c in self.constraints[-5:]:  # 最多显示 5 条
                lines.append(f"- {c}")
        ...
        if self.artifacts:
            lines.append("")
            lines.append("### 已有产物")
            for a in self.artifacts[-5:]:  # 最多显示 5 条
