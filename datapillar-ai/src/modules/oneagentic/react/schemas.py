...
PlanStatus = Literal["planning", "executing", "reflecting", "completed", "failed"]
...
class Plan(BaseModel):
    ...
    status: PlanStatus = Field(default="planning", description="计划状态")
    ...
    def get_completed_task_ids(self) -> set[str]:
        """获取已完成的任务 ID"""
        return {t.id for t in self.tasks if t.status == "completed"}
    ...
    def to_prompt(self) -> str:
        ...
        for task in self.tasks:
            status_marker = f"[{task.status}]"
            deps = f" (依赖: {', '.join(task.depends_on)})" if task.depends_on else ""
            line = (
                f"- {status_marker} [{task.id}] {task.description} -> {task.assigned_agent}{deps}"
            )
