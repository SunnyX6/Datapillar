...
使用示例：
```python
from src.modules.oneagentic.react.planner import create_plan
...
from src.modules.oneagentic.react.schemas import Plan, PlannerOutput
...
输出：
{{
  "understanding": "用户想要创建一个用户宽表，需要先分析需求，设计表结构，然后生成 SQL",
  "tasks": [
    {{"description": "分析用户需求，确定宽表需要哪些字段", "assigned_agent": "analyst", "depends_on": []}},
    {{"description": "设计宽表的技术方案和 ETL 流程", "assigned_agent": "architect", "depends_on": ["1"]}},
    {{"description": "生成创建宽表的 SQL 脚本", "assigned_agent": "developer", "depends_on": ["2"]}}
  ]
}}
"""
...
async def create_plan(
    *,
    ...
    # 过滤掉入口 Agent
    available_agents = [a for a in available_agents if not a.is_entry]
