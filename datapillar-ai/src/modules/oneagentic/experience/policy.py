...
class FeedbackAwareSedimentationPolicy(SedimentationPolicy):
    ...
    async def _get_task_rejection_rate(self, task_type: str) -> float | None:
        """获取任务类型的历史拒绝率"""
        from src.modules.oneagentic.experience.episode import ValidationStatus
        from src.modules.oneagentic.experience.store import SearchFilter

        # 查询该任务类型的历史经验
        filter_obj = SearchFilter(task_type=task_type)
        episodes = await self._store.list(filter=filter_obj, limit=20)

        if len(episodes) < self._min_history_count:
            return None
        ...
        rejected_count = sum(
            1 for e in with_feedback if e.validation_status == ValidationStatus.REJECTED
        )
    ...
    async def _get_user_avg_satisfaction(self, user_id: str) -> float | None:
        ...
        filter_obj = SearchFilter(user_id=user_id)
        episodes = await self._store.list(filter=filter_obj, limit=20)
