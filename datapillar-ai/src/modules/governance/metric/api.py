...
async def ai_fill(request: Request, payload: AIFillRequest):
    ...
    result: AIFillResponse = await metric_ai_service.fill(payload, tenant_id=current_user.tenant_id)
    return ApiResponse.success(request=request, data=result.model_dump())
