...
class SseAgent(BaseModel):
    id: str = Field(..., description="内部节点ID：knowledge_agent/analyst_agent 等")
    name: str = Field(..., description="展示名：知识检索专家/需求分析师 等")
...
class SseMessage(BaseModel):
    ...
    content: str = Field(..., description="用户可见文本（可选，仅关键摘要）")


class SseTool(BaseModel):
    name: str = Field(..., description="工具名")
    input: Any | None = Field(default=None, description="工具入参（可截断/脱敏）")
    output: Any | None = Field(default=None, description="工具输出（可截断/脱敏）")
...
class SseError(BaseModel):
    ...
    detail: str | None = Field(default=None, description="错误详情（可选）")


class SseEvent(BaseModel):
    """
    SSE 事件统一结构（data payload）
    ...
    def result_event(cls, *, workflow: dict[str, Any] | None, message: str = "生成完成") -> "SseEvent":
        return cls(
            ...
            result=SseResult(workflow=workflow),
        )
