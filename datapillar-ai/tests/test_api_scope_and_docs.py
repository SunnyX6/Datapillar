...
def test_ai_router_uses_biz_or_admin_scope_prefix() -> None:
    ...
    assert "/admin/openlineage" in ai_paths
    assert "/biz/openlineage" not in ai_paths
...
def test_app_registers_scalar_and_scoped_openapi_routes() -> None:
    ...
    assert "/api/ai/admin/openlineage" in route_paths
    assert "/api/ai/biz/openlineage" not in route_paths
