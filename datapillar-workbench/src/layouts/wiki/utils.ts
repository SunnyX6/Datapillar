...
import type { ChunkApi, DocumentApi, NamespaceApi, RetrieveHitApi } from '@/services/knowledgeWikiService'
...
export const mapNamespaceToSpace = (item: NamespaceApi, color: string): KnowledgeSpace => {
  return {
...
export const mapDocumentToUi = (item: DocumentApi): Document => {
  return {
...
export const mapChunkToUi = (item: ChunkApi): Chunk => {
  return {
    id: item.chunk_id,
    docId: item.doc_id,
...
export const mapSearchHitToResult = (hit: RetrieveHitApi): SearchResult => {
  return {
