...
export function ChatWidget() {
  ...
  const fallbackResponses = useMemo(() => t('chat.fallbackResponses', { returnObjects: true }) as string[], [t])
  ...
  const hasOnlyInitialMessage = messages.length === 1 && messages[0]?.role === 'model'
  const displayMessages = hasOnlyInitialMessage
    ? [{ role: 'model', text: initialMessage }]
    : messages
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    setInput('')
    setMessages((prev) => {
      const isInitialOnly = prev.length === 1 && prev[0]?.role === 'model'
      const normalizedPrev = isInitialOnly
        ? [{ role: 'model', text: initialMessage }]
        : prev
      return [...normalizedPrev, { role: 'user', text: userMessage }]
    })
    setIsLoading(true)
            ...
            {displayMessages.map((msg, idx) => (
              <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                <div className={`flex flex-col max-w-[80%] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                  ...
                  <div
                    className={`px-4 py-2.5 text-body-sm leading-relaxed shadow-sm ${
                      msg.role === 'user'
            ...
            <form onSubmit={handleSubmit} className="relative group">
              <div className="relative bg-[#1e1e1e] rounded-3xl border border-slate-700/50 shadow-lg focus-within:border-slate-600 focus-within:ring-1 focus-within:ring-slate-600 transition-all overflow-hidden">
                <input
                  ...
                  className="w-full bg-transparent border-none text-white text-body-sm px-4 py-3.5 pr-12 focus:outline-none placeholder:text-slate-500"
                />
                <button
                  type="submit"
