...
const PIPELINE_TYPING_SPEED = 38
const ANALYTICS_TYPING_SPEED = 36

// Keep scenario switch aligned with inner animation timeline.
const PIPELINE_TIMELINE_OFFSET = 4200
const ANALYTICS_TIMELINE_OFFSET = 1400

const getPipelineScenarioDuration = (prompt: string) =>
  Math.max(6800, getTypingDuration(prompt, PIPELINE_TYPING_SPEED) + PIPELINE_TIMELINE_OFFSET + 900)

const getAnalyticsScenarioDuration = (prompt: string) =>
  Math.max(5200, getTypingDuration(prompt, ANALYTICS_TYPING_SPEED) + ANALYTICS_TIMELINE_OFFSET + 1200)
...
export function Hero({ onRequestAccess }: HeroProps) {
  ...
  const pipelinePrompt = t('hero.pipeline.prompt')
  const analyticsPrompt = t('hero.analytics.prompt')
    ...
    () => [
      {
        ...
        duration: getPipelineScenarioDuration(pipelinePrompt)
      },
      {
        id: 'analytics',
        ...
        duration: getAnalyticsScenarioDuration(analyticsPrompt)
      }
    ],
    [t, pipelinePrompt, analyticsPrompt]
  )
...
const PipelineView = () => {
  ...
  const typingSpeed = PIPELINE_TYPING_SPEED
...
const AnalyticsView = () => {
  ...
  const typingSpeed = ANALYTICS_TYPING_SPEED
  const typingDuration = getTypingDuration(prompt, typingSpeed)
  const isPendingInput = step < 1
  const showUserMessage = step >= 1
  const showTyping = step >= 2 && step < 3
