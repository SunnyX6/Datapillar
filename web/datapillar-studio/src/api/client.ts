...
import { handleAppError } from '@/api/errorCenter'
import {
...
import { normalizeApiPayloadFailure, normalizeAxiosFailure } from './errorNormalizer'
import { attachCsrfHeader } from './interceptors'
...
refreshClient.interceptors.request.use((config) => {
  ...
  attachCsrfHeader(headers, {
    cookieName: REFRESH_CSRF_COOKIE_NAME,
    headerName: REFRESH_CSRF_HEADER_NAME
  })
...
function handleApiPayloadFailure(
  ...
  handleAppError(
    normalizeApiPayloadFailure(payload, {
      module: 'api/client',
      status,
      requestUrl,
      method,
