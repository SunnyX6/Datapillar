...
function buildAppError(source:AppErrorSource,message:string,options:NormalizeOptions):AppError {
 return {
 source,module:resolveModule(options.module,options.route),message,retryable:false,severity:source === 'runtime' || source === 'router'?'fatal':'error',isCoreRequest:options.isCoreRequest?? false,url:options.requestUrl,method:options.method,raw:options.raw
 }
}

export function normalizeApiPayloadError(payload:ApiResponse<unknown> | ErrorResponse,options:NormalizeOptions & { status?: number;headers?: unknown }):AppError {
 ...
 const message = extractMessage(payload)?? (code!== undefined?`Request failed (code:${code})`:'Request failed')
 ...
 return {...buildAppError('axios',message,options),code,errorType,status:options.status,retryable:payloadRetryable?? (typeof options.status === 'number'?options.status >= 500:false),severity:typeof options.status === 'number' && options.status >= 500?'error':'warn',context,requestId:headerRequestId?? payloadRequestId,traceId:headerTraceId?? payloadTraceId,raw:payload
 }
}
...
export function normalizeAxiosError(error:unknown,options:NormalizeOptions):AppError {
 ...
 return {...buildAppError('axios',message,{...options,requestUrl,method }),code,status,retryable,severity:typeof status === 'number'?(status >= 500?'error':'warn'):'error',requestId,traceId,raw:error
 }
}

export function normalizeFetchResponseError(response:Response,options:NormalizeOptions):AppError {
 ...
 return {...buildAppError('fetch',message,options),status,code:status,retryable:status >= 500,severity:status >= 500?'error':'warn',requestId,traceId,raw:response
 }
}

export function normalizeUnknownError(error:unknown,options:NormalizeOptions):AppError {
 return {...buildAppError('unknown',normalizeErrorMessage(error),options),retryable:false,severity:'error',raw:options.raw?? error
 }
}

export function normalizeRuntimeError(error:unknown,options:NormalizeOptions):AppError {
 return {...buildAppError('runtime',normalizeErrorMessage(error),options),severity:'fatal',retryable:false,raw:options.raw?? error
 }
}

export function normalizeRouteError(error:unknown,options:NormalizeOptions):AppError {
 if (error && typeof error === 'object' && 'status' in error) {
 const candidate = error as { status?: unknown;statusText?: unknown;data?: unknown }
 const status = typeof candidate.status === 'number'?candidate.status:undefined
 const fallbackMessage = toStringValue(candidate.statusText)?? 'Route loading failed'
 const message = extractMessage(candidate.data)?? fallbackMessage
 return {...buildAppError('router',message,options),status,code:status,severity:'fatal',retryable:status!== undefined?status >= 500:false,raw:options.raw?? error
 }
 }

 return {...buildAppError('router',normalizeErrorMessage(error),options),severity:'fatal',retryable:false,raw:options.raw?? error
 }
}
