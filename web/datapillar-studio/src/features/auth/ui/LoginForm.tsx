 ...
 *
 * Function:* 1.Username/Password login
 * 2.theme switching
 * 3.form validation
 * 4.Error handling
 */
...
import { useThemeMode,useAuthStore } from '@/state'
...
import { buildEntryPathWithFrom,buildLocationPath } from '@/utils/exceptionNavigation'
import { isTenantSelectResult,type LoginResult,type TenantOption } from '@/services/types/auth'
import { WorkspaceSelectPanel } from './WorkspaceSelect'

/**
 * Dimensional configuration system
 * by adjusting SCALE value to uniformly control the size of all components
 * SCALE = 1.0 (Default),1.1 (Zoom in10%),0.9 (zoom out10%)
 */
const SCALE = 1.3 // Adjust this value to make all components larger or smaller

// Form base size(for adaptive scaling)
...
const SIZES = {
 // Input box
 input:{
 py:SCALE >= 1.15?'py-2.5':SCALE >= 1.0?'py-2':'py-1.5',px:SCALE >= 1.15?'pl-12 pr-10':SCALE >= 1.0?'pl-12 pr-10':'pl-10 pr-8',text:SCALE >= 1.15?'text-sm':SCALE >= 1.0?'text-sm':'text-xs',iconSize:Math.round(12 * SCALE),iconPadding:SCALE >= 1.15?'pl-3.5':'pl-3.5'
 },// button
 button:{
 py:SCALE >= 1.15?'py-2.5':SCALE >= 1.0?'py-2':'py-1.5',text:SCALE >= 1.15?'text-sm':SCALE >= 1.0?'text-sm':'text-xs',iconSize:Math.round(16 * SCALE)
 },// Logo
 logo:{
 size:SCALE >= 1.15?'w-9 h-9':SCALE >= 1.0?'w-9 h-9':'w-8 h-8',iconSize:Math.round(18 * SCALE)
 },// Title
 title:{
 text:SCALE >= 1.15?'text-xl':SCALE >= 1.0?'text-xl':'text-lg'
 },// checkbox
 checkbox:{
 size:SCALE >= 1.15?'h-3.5 w-3.5':SCALE >= 1.0?'h-3 w-3':'h-2.5 w-2.5',iconSize:Math.round(10 * SCALE)
 }
}
...
const PROVIDER_CONFIG:Array<{
 ...
 enabled:boolean
}> = [{ key:'dingtalk',nameKey:'sso.providers.dingtalk.name',descKey:'sso.providers.dingtalk.desc',icon:'DingTalk',enabled:false },{ key:'wecom',nameKey:'sso.providers.wecom.name',descKey:'sso.providers.wecom.desc',icon:'WeCom',enabled:false },{ key:'lark',nameKey:'sso.providers.lark.name',descKey:'sso.providers.lark.desc',icon:'Lark',enabled:false }]
...
export function LoginFormContent({ tenantCode,onSsoClick,onLoginSuccess }:LoginFormContentProps) {
 ...
 const rememberHintContent = (<div className="flex flex-col gap-0.5">
 ...
 mode === 'dark'?{
 background:'#0F172A',color:'#E2E8F0',border:'1px solid rgba(129,140,248,0.5)',boxShadow:'0 18px 45px rgba(15,23,42,0.55)'
 }:{
 background:'#EEF2FF',color:'#312E81',border:'1px solid rgba(79,70,229,0.15)',boxShadow:'0 12px 30px rgba(79,70,229,0.15)'
 }
 ...
 */
 ...
 if (!isTenantSelectResult(result)) {
 navigate(buildEntryPathWithFrom(buildLocationPath(location)),{ replace:true })
 }
 } catch (error) {
 ...
 rememberMe?'border-transparent bg-gradient-to-br from-indigo-500 to-purple-500 text-white shadow-[0_8px_14px_rgba(79,70,229,0.35)]':'border-slate-400/70 dark:border-slate-600 bg-transparent text-transparent group-hover:border-indigo-400/70 dark:group-hover:border-indigo-500/60'
 }`}
 >
 ...
 className={cn('flex h-3.5 w-3.5 items-center justify-center rounded-full border border-slate-300/70 text-slate-400 dark:border-slate-600 dark:text-slate-500 transition-colors',loading?'cursor-not-allowed':'cursor-help hover:text-slate-600 dark:hover:text-slate-300 hover:border-slate-400/80 dark:hover:border-slate-500')}
 >
 ...
 type="submit"
 disabled={loading}
 // Login page buttons should not inherit Button the default header Size(Contains @md:py-* Container breakpoint),Otherwise,the customization here will be overwritten.py-*.size="normal"
 className={cn('w-full text-white rounded-lg shadow-md shadow-indigo-500/20 disabled:cursor-not-allowed','flex items-center justify-center gap-2','bg-indigo-500 hover:bg-indigo-600 active:bg-indigo-700 disabled:bg-indigo-400','dark:bg-indigo-500 dark:hover:bg-indigo-600 dark:active:bg-indigo-700 dark:disabled:bg-indigo-400','transition-colors duration-150',SIZES.button.text,SIZES.button.py,'font-semibold','-mt-1','focus-visible:outline focus-visible:outline-2 focus-visible:outline-indigo-400')}
 >
 {loading?(<>
 ...
 {onSsoClick?(<div className="flex justify-center mt-3">
 <button
 type="button"
 onClick={onSsoClick}
 disabled={loading}
 className={cn(TYPOGRAPHY.legal,'group inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1.5 font-semibold text-slate-700','hover:border-indigo-300 hover:bg-indigo-50/40 hover:text-slate-900','dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800/80','focus-visible:outline focus-visible:outline-2 focus-visible:outline-indigo-400',loading?'cursor-not-allowed opacity-60':'transition-colors')}
 >
...
export function LoginForm({ scale,ready,tenantCode }:LoginFormProps) {
 ...
 transform:`scale(${scale})`,transformOrigin:'center center',opacity:ready?1:0,transition:'opacity 0.3s ease'
 }),[scale,ready])
 ...
 if (isTenantSelectResult(result)) {
 toast.error('Tenant selection not completed,Please try again.')
 return
 }
 setTenantOptions([])
 navigate(buildEntryPathWithFrom(buildLocationPath(location)),{ replace:true })
 ...
 >
 <div
 className={cn('relative flex flex-col gap-3 md:gap-4 lg:gap-5 rounded-none w-full',showWorkspaceSelect?'h-[var(--login-form-base-height)]':'min-h-[var(--login-form-base-height)]',paddingClassMap.md,LOGIN_FORM_WIDTH_CLASS)}
 style={panelStyle}
 >
 {showWorkspaceSelect?(<WorkspaceSelectPanel
 tenants={tenantOptions}
 onBack={handleWorkspaceBack}
 onSelect={handleWorkspaceSelect}
 loading={tenantSelectLoading}
 />):(<>
 {/* brand area - Fixed height to prevent layout jitter when switching between Chinese and English */}
 ...
 {activeTab === 'account'?(<LoginFormContent
 ...
 className={cn('h-[200px] overflow-y-auto overscroll-contain scrollbar-invisible py-1 flex flex-col transition-opacity',showListPanel?'opacity-100':'opacity-0 pointer-events-none')}
 >
 {PROVIDER_CONFIG.map((provider) => {
 const Icon = ThirdPartyIcons[provider.icon]
 ...
 className={cn('group relative rounded-2xl px-4 py-3 flex items-center gap-3 text-left transition-colors','bg-transparent text-slate-600 dark:text-slate-300',disabled?'opacity-60 cursor-not-allowed':'cursor-pointer hover:bg-white dark:hover:bg-slate-900/80 hover:shadow-[inset_0_0_0_1px_rgba(226,232,240,0.9)] dark:hover:shadow-[inset_0_0_0_1px_rgba(51,65,85,0.8)] hover:z-10')}
 >
 ...
 {!provider.enabled && (<span className={`${TYPOGRAPHY.micro} font-semibold text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full`}>
 ...
 {!disabled && (<div className="ml-auto flex h-8 w-8 items-center justify-center rounded-full bg-indigo-50 text-indigo-600 border border-indigo-100 opacity-0 transition-opacity duration-150 group-hover:opacity-100 dark:bg-indigo-500/10 dark:text-indigo-300 dark:border-indigo-500/30">
 ...
 {/* Copyright area at the bottom - Fixed height to prevent layout jitter when switching between Chinese and English */}
 {!showWorkspaceSelect?(<div className="text-legal text-slate-400 dark:text-slate-500 text-center mt-3 select-none min-h-[48px]">
