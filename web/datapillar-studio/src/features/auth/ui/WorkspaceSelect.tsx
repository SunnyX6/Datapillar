...
const toneClassMap:Record<WorkspaceItem['tone'],string> = {
 primary:'bg-indigo-500 text-white',dark:'bg-slate-900 text-white',muted:'bg-slate-200 text-slate-600 dark:bg-slate-800 dark:text-slate-400'
}
...
export function WorkspaceSelectPanel({ tenants,onBack,onSelect,loading = false }:WorkspaceSelectPanelProps) {
 ...
 const name = tenant.tenantName?.trim() || tenant.tenantCode?.trim() || `tenant-${tenant.tenantId}`
 const iconText = name.slice(0,1)
 const isDefault = tenant.isDefault === 1
 return {
 id:tenant.tenantId,name,iconText,metaPrimary:tenant.tenantCode || `ID:${tenant.tenantId}`,metaSecondary:isDefault?'Default':'member',metaPrimaryTone:'accent',metaSecondaryTone:isDefault?'accent':'muted',tone:isDefault?'primary':'dark',highlight:isDefault,disabled:tenant.status!== 1
 }
 })
 },[tenants])

 return (<div className="flex flex-col gap-6 h-full min-h-0">
 <button
 type="button"
 onClick={onBack}
 ...
 <div>
 <h2 className="text-xl font-semibold text-slate-900 dark:text-white">Select workspace</h2>
 <p className="mt-2 text-xs text-slate-400 dark:text-slate-500">Your account is associated with the following organizational nodes:</p>
 </div>
 ...
 >
 {workspaces.length === 0?(<div className="px-4 py-6 text-xs text-slate-400 dark:text-slate-500">
 No available tenants yet
 ...
 className={cn('group relative flex w-full items-center gap-3 px-4 py-3 text-left transition-colors cursor-pointer rounded-2xl','bg-transparent text-slate-600 hover:bg-white dark:text-slate-300 dark:hover:bg-slate-900/80','hover:shadow-[inset_0_0_0_1px_rgba(226,232,240,0.9)] dark:hover:shadow-[inset_0_0_0_1px_rgba(51,65,85,0.8)] hover:z-10',(loading || workspace.disabled) && 'opacity-60 cursor-not-allowed')}
 >
 ...
 Cross-tenant login enabled.After entering,you can quickly switch through the avatar drop-down.</p>
 </div>
