...
import {
 Activity,ArrowUpRight,Award,Clock3,Cpu,Globe,Layers,Mail,MessageSquare,ShieldCheck,Sparkles,Users
} from 'lucide-react'
...
const STAT_METRICS:StatMetric[] = [{
 label:'Active Pipelines',value:'28',subLabel:'99.1% success',change:'+3.8%',isPositive:true,data:[18,20,19,21,22,24,23,25,26],color:'#6366f1',icon:<Activity size={16} className="text-indigo-500" />
 },{
 label:'AI Assisted Fixes',value:'126',subLabel:'Last 30 days',change:'+18%',isPositive:true,data:[4,6,5,8,10,9,11,13,12],color:'#22c55e',icon:<Sparkles size={16} className="text-emerald-500" />
 },{
 label:'Spend vs Budget',value:'$82k',subLabel:'Budget $96k',change:'-15%',isPositive:true,data:[95,92,91,90,88,86,84,83,82],color:'#f59e0b',icon:<Cpu size={16} className="text-amber-500" />
 }]
...
const JOURNEY_STEPS:JourneyStep[] = [{
 title:'Zero-copy ingestion fabric',detail:'Kafka,MySQL sources unified into Delta mesh',time:'Completed in 2 weeks ago',status:'done'
 },{
 title:'AI guardrails rollout',detail:'42 anomaly rules monitored by Datapillar AI',time:'Rolling out to finance zone',status:'in-progress'
 },{
 title:'Self-serve governance',detail:'Federated access policies + audit automation',time:'Next milestone.Q4',status:'upcoming'
 }]
...
const PROJECT_FOCUS:ProjectFocus[] = [{
 name:'Composable Data Products',owner:'Team Nexus',health:'On Track',risk:'Latency budget 120ms'
 },{
 name:'Governed AI Studio',owner:'ModelOps Crew',health:'Watch',risk:'GPU quota @ 72%'
 },{
 name:'Usage Based Billing',owner:'RevOps Guild',health:'On Track',risk:'Needs lineage handshake'
 }]
...
const CONTACTS:Contact[] = [{
 name:'Mila Tang',role:'Data Product Lead',channel:'Slack #datagov',eta:'response < 10 minutes'
 },{
 name:'Raj Patel',role:'ML Platform',channel:'PagerDuty.Platform',eta:'24×7 on-call'
 },{
 name:'Aurora Ops',role:'Support Bot',channel:'Email support@datapillar.ai',eta:'SLA 4 hours'
 }]
...
export function ProfileLayout() {
 ...
 return (<section className="flex h-full w-full overflow-hidden bg-white dark:bg-slate-900 selection:bg-indigo-500/30 @container">
 ...
 {STAT_METRICS.map((metric) => (<BentoCard key={metric.label} className="col-span-12 @md:col-span-4">
...
function ProfileHeader({ profile }:{ profile:StudioUserProfile | null }) {
 ...
 return email || 'No email has been set yet'
 },[profile?.email])

 const profilePhone = useMemo(() => {
 const phone = profile?.phone?.trim()
 return phone || 'No mobile phone number has been set yet'
 },[profile?.phone])
 ...
 Work email:<span className="font-medium text-slate-700 dark:text-slate-200">{profileEmail}</span>
 {'.'}
 mobile phone:<span className="font-medium text-slate-700 dark:text-slate-200">{profilePhone}</span>
 </p>
 <p className="text-body-sm text-slate-500 dark:text-slate-400 mt-2 max-w-3xl">
 Owns the federated data operating model for <span className="font-medium text-indigo-600 dark:text-indigo-300">Acme Corp</span>,orchestrating ingestion,AI quality guardrails,and governed activation layers.</p>
 <div className="flex flex-wrap gap-2 mt-3">
 {PROFILE_TAGS.map((tag) => (<span key={tag} className="px-2.5 py-1 rounded-full bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-200 text-legal font-semibold">
 {tag}
 </span>))}
 </div>
 </div>
...
function QuickStat({ label,value,icon }:{ label:string;value:string;icon:ReactNode }) {
 return (<div className="flex items-center gap-3 p-2.5 rounded-2xl bg-slate-50 dark:bg-slate-900/40 border border-slate-100 dark:border-white/5">
...
function JourneyTimeline() {
 ...
 {JOURNEY_STEPS.map((step,index) => (<div key={step.title} className="flex gap-4">
...
function ProjectFocusPanel() {
 ...
 {PROJECT_FOCUS.map((project) => (<div key={project.name} className="p-3 rounded-2xl border border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-slate-900/40">
 <div className="flex items-center justify-between">
 <div>
...
function HighlightCard({ title,value,description,pill }:{ title:string;value:string;description:string;pill:string }) {
 return (<div className="p-3 rounded-2xl border border-slate-100 dark:border-white/5 bg-white dark:bg-slate-900/60 shadow-sm">
 <div className="flex items-center justify-between">
...
function SupportContacts() {
 ...
 <div className="mt-4 space-y-3">
 {CONTACTS.map((contact) => (<div key={contact.name} className="p-3 rounded-2xl border border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-slate-900/40">
 <div className="flex items-center justify-between">
 <div>
...
function Sparkline({ data,color }:SparklineProps) {
 ...
 return (<svg width="100%" height="36" viewBox={`0 0 ${width} ${height}`} className="overflow-visible">
