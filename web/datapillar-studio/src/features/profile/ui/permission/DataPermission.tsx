...
import type { UserDataPrivilege } from '../../utils/permissionTypes'
...
const MOCK_GRAVITINO_ASSETS:GravitinoAsset[] = [{
 id:'metalake_prod',name:'Prod Metalake',type:'metalake',description:'Collection of production environment data assets and platform metadata.',children:[{
 id:'cat_pg_payment',name:'pg_payment',type:'catalog',description:'Pay domain home directory.',children:[{
 id:'sch_core',name:'core',type:'schema',description:'Core business data model.',children:[{
 id:'tbl_orders',name:'fact_orders',type:'table',description:'Order details fact table.'
 },{
 id:'tbl_refunds',name:'fact_refunds',type:'table',description:'Refund details fact sheet.'
 }]
 },{
 id:'sch_dim',name:'dim',type:'schema',description:'Common Dimension Data Schema.',children:[{
 id:'tbl_users',name:'dim_users',type:'table',description:'User dimension table.'
 }]
 }]
 },{
 id:'cat_pg_marketing',name:'pg_marketing',type:'catalog',description:'Marketing Domain Data Asset Catalog.',children:[{
 id:'sch_ads',name:'ads',type:'schema',description:'Advertising data model.',children:[{
 id:'tbl_campaigns',name:'fact_campaigns',type:'table',description:'Advertising campaign fact sheet.'
 }]
 }]
 }]
 },{
 id:'metalake_analytics',name:'Analytics Metalake',type:'metalake',description:'Analysis andBICollection of related data assets.',children:[{
 id:'cat_warehouse',name:'warehouse',type:'catalog',description:'Data warehouse unified directory.',children:[{
 id:'sch_sales',name:'sales',type:'schema',description:'sales data schema.',children:[{
 id:'tbl_sales_daily',name:'fact_sales_daily',type:'table',description:'Daily sales fact table.'
 }]
 }]
 }]
 }]
...
export function DataPermission({ subject,onPrivilegesChange }:DataPermissionProps) {
 ...
 onPrivilegesChange?.(Object.entries(nextPrivilegesMap).map(([assetId,privileges]) => ({
 assetId,privileges
 })))
 ...
 const colorClass =
 node.type === 'metalake'?'text-indigo-600 dark:text-indigo-400':node.type === 'catalog'?'text-blue-600 dark:text-blue-400':node.type === 'schema'?'text-amber-500 dark:text-amber-400':'text-slate-500 dark:text-slate-400'
 ...
 className={cn('flex items-center gap-1.5 py-1.5 px-2 cursor-pointer select-none transition-colors border-l-2',isSelected?'bg-brand-50 border-brand-500 text-brand-900 font-medium dark:bg-brand-500/10 dark:border-brand-400/60 dark:text-brand-200':'border-transparent text-slate-600 hover:bg-slate-50 hover:text-slate-900 dark:text-slate-300 dark:hover:bg-slate-800/60 dark:hover:text-slate-100')}
 ...
 return (<Card variant="default" padding="none" className="flex h-full overflow-hidden shadow-none dark:shadow-none">
 ...
 {selectedAsset?(<div className="flex flex-col h-full animate-in fade-in duration-200">
 <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800">
 <div className="flex items-center justify-between">
 <div className="flex items-center gap-3">
 <div
 className={cn('p-2 rounded-lg',selectedAsset.type === 'metalake'?'bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-300':selectedAsset.type === 'catalog'?'bg-blue-50 text-blue-600 dark:bg-blue-500/10 dark:text-blue-300':selectedAsset.type === 'schema'?'bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-300':'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300')}
 >
 {selectedAsset.type === 'metalake'?(<Server size={20} />):selectedAsset.type === 'catalog'?(<Database size={20} />):selectedAsset.type === 'schema'?(<Folder size={20} />):(<Table size={20} />)}
 </div>
 <div>
 ...
 {privilegeGroups.map((group) => (<Card key={group.name} variant="default" padding="none" className="rounded-xl overflow-hidden shadow-none dark:shadow-none">
 ...
 <h5
 className={cn('text-xs font-bold uppercase tracking-wider flex items-center gap-2',group.danger?'text-rose-600 dark:text-rose-400':'text-slate-500 dark:text-slate-400')}
 >
 ...
 return (<div
 key={item.key}
 className={cn('flex items-center justify-between px-5 py-4 transition-all duration-200',isActive?'bg-slate-50/50 dark:bg-slate-800/60':'hover:bg-slate-50/30 dark:hover:bg-slate-800/70')}
 >
 <div className="flex items-start gap-4">
 <div
 className={cn('mt-1 p-1.5 rounded-md transition-colors',isActive?'bg-white dark:bg-slate-900 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700':'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500')}
 >
 ...
 <span
 className={cn('text-legal font-bold font-mono',isActive?'text-slate-900 dark:text-slate-100':'text-slate-600 dark:text-slate-300')}
 >
 {item.label}
 </span>
