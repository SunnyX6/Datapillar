...
import type {
 AiAccessLevel,RoleDefinition,UserItem,UserStatus,} from '../../utils/permissionTypes'
import { InviteMemberModal } from './InviteMemberModal'
import { PermissionAvatar } from './PermissionAvatar'
import { UserPermissionDrawer } from './UserPermissionDrawer'

const STATUS_STYLES:Record<UserStatus,string> = {
 Activated:'bg-emerald-50 text-emerald-700 border-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-300 dark:border-emerald-500/30',Invited:'bg-amber-50 text-amber-700 border-amber-100 dark:bg-amber-500/10 dark:text-amber-300 dark:border-amber-500/30',Disabled:'bg-rose-50 text-rose-700 border-rose-100 dark:bg-rose-500/10 dark:text-rose-300 dark:border-rose-500/30',}
...
interface MembersListProps {
 ...
 users:UserItem[]
 onUpdateUserModelAccess:(userId:string,aiModelId:number,access:AiAccessLevel,) => void
...
export function MembersList({
 ...
 const selectableUsers = useMemo(() => assignedUsers.filter((user) =>!isPlatformSuperAdmin(user)),[assignedUsers],)
 ...
 {showToolbar && (<div className="flex items-end justify-between gap-4">
 <div>
 <h3
 ...
 {assignedUsers.length === 0?(<TableRow>
 ...
 </TableRow>):(assignedUsers.map((user) => {
 ...
 className={cn('transition-colors',isProtectedUser?'cursor-default':'cursor-pointer',isSelected?'bg-brand-50/55 dark:bg-brand-500/10':isProtectedUser?'':'hover:bg-slate-50/80 dark:hover:bg-slate-800/70',)}
 >
 <TableCell className="px-6 py-4">
 <div className="flex items-center gap-3">
 {isSelected?(<span className="w-8 h-8 rounded-full bg-brand-600 text-white flex items-center justify-center shadow-sm shrink-0">
 <Check size={14} />
 </span>):(<PermissionAvatar
 name={user.name}
 src={user.avatarUrl}
 size="sm"
 />)}
 <div>
 <div
 className={cn(TYPOGRAPHY.bodySm,'font-medium text-slate-900 dark:text-white',)}
 >
 {user.name}
 </div>
 <div
 ...
 <TableCell className="px-6 py-4">
 <div
 className={cn(TYPOGRAPHY.caption,'flex items-center gap-2 text-slate-500 dark:text-slate-400',)}
 >
 <Building2
 size={12}
 ...
 <TableCell
 className={cn(TYPOGRAPHY.caption,'px-6 py-4 text-slate-500 dark:text-slate-400 font-mono',)}
 >
 {user.lastActive}
 </TableCell>
 <TableCell className="px-6 py-4 text-center">
 {!isProtectedUser && (<Button
 size="small"
 variant="outline"
 onClick={(event) => {
 ...
 {selectedCount > 0 && (<div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 animate-in slide-in-from-bottom-3 fade-in duration-200">
 ...
 {selectedUser && (<UserPermissionDrawer
 isOpen={!!selectedUser}
