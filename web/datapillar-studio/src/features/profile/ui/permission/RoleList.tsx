...
interface RoleListProps {
 ...
 onUpdateRole?: (roleId:string,payload:{
 name:string
 description?: string
 type:RoleType
 }) => Promise<boolean> | boolean
...
export function RoleList({
 ...
 editingRole?roles.filter((role) => role.id!== editingRole.id).map((role) => role.name):[],[editingRole,roles],)
 ...
 return (<div className="w-80 flex flex-col border-r border-slate-200 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-900/70 h-full">
 ...
 isSelected?'bg-white dark:bg-slate-900 ring-1 ring-slate-200 dark:ring-slate-700 shadow-sm':'hover:bg-slate-100/80 dark:hover:bg-slate-800/80'
 }`}
 >
 ...
 <span
 className={cn(TYPOGRAPHY.bodyXs,isSelected?'text-slate-900 dark:text-white font-semibold':'text-slate-600 dark:text-slate-300 font-medium',)}
 >
 {role.name}
 </span>
 </div>
 ...
 <span
 className={cn('w-1.5 h-1.5 rounded-full',role.isSystem || role.userCount > 0?'bg-emerald-400':'bg-slate-300',)}
 />
 </div>
 </button>
 <div
 className={cn('flex items-center gap-1 pt-0.5 transition-opacity',isSelected?'opacity-100 pointer-events-auto':'opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto',)}
 >
 ...
 className={cn('rounded-md p-1 transition-colors',canEditRole?'text-slate-400 hover:text-slate-700 hover:bg-slate-200/70 dark:text-slate-500 dark:hover:text-slate-200 dark:hover:bg-slate-700':'text-slate-300 dark:text-slate-700 cursor-not-allowed',)}
 >
 <Pencil size={14} />
 </button>
 </Tooltip>
 ...
 className={cn('rounded-md p-1 transition-colors',canDeleteRole?'text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:text-slate-500 dark:hover:text-rose-400 dark:hover:bg-rose-500/10':'text-slate-300 dark:text-slate-700 cursor-not-allowed',)}
 >
 <Trash2 size={14} />
 </button>
 </Tooltip>
 <ChevronRight
 size={14}
 className={cn(isSelected?'text-slate-500':'text-slate-300',)}
 />
 </div>
 </div>
 </div>)
