...
export function FunctionalPermission(props:FunctionalPermissionProps) {
 ...
 const renderPermissionNodes = (nodes:PermissionItem[],depth = 0,):ReactElement[] => {
 ...
 return (<div key={permission.objectId}>
 <div
 ...
 </button>):depth > 0?(<span className="inline-block h-3 w-3 rounded-bl-sm border-l border-b border-slate-200 dark:border-slate-700" />):(<span className="inline-block h-3 w-3" />)}
 </div>
 <div className="min-w-0">
 <div
 className={cn(TYPOGRAPHY.bodySm,'font-medium text-slate-900 dark:text-white mb-0.5 truncate',)}
 >
 ...
 {hasChildren &&!isCollapsed?renderPermissionNodes(permission.children,depth + 1):null}
 </div>)
 })
 }

 return (<div className={cn('space-y-4',props.className)}>
 {Object.entries(groupedPermissions).map(([category,categoryPermissions]) => (<div key={category}>
