  ...
  const releaseStages = [
    {
      ...
      subtitle: 'Full quantity available',
      icon: ShieldCheck,
    ...
    },
    {
      id: 'deprecated',
      title: 'abandoned',
      subtitle: 'Planned to go offline',
      icon: Clock,
  ...
  return (
    <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
      <div className="space-y-2">
        <div className="px-1">
          <h3 className={sectionTitleClassName}>Basic attributes</h3>
        </div>
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-1 grid grid-cols-2 gap-x-6 gap-y-4">
              <div className="space-y-2 col-span-2 @md:col-span-1">
                ...
                >
                  display name <Info size={10} className="text-slate-300 dark:text-slate-500" />
                </label>
                <input
                  type="text"
              ...
              <div className="space-y-2 col-span-2">
                ...
                <textarea
                  rows={2}
                  placeholder="Describe in detail the business scenario and permission boundaries of the resource..."
                  className={cn(
                    TYPOGRAPHY.bodyXs,
      ...
      <div className="space-y-2">
        <div className="px-1">
          <h3 className={sectionTitleClassName}>Release life cycle (STATUS)</h3>
        </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between px-1">
          <h3 className={sectionTitleClassName}>action list (Action Manifest)</h3>
          <button
            type="button"
            className={cn(
              TYPOGRAPHY.micro,
          ...
          <table className="w-full border-collapse">
            <thead className="bg-slate-50 dark:bg-slate-800/50 text-slate-400 dark:text-slate-400 uppercase tracking-wider border-b border-slate-100 dark:border-slate-700">
              <tr className={cn(TYPOGRAPHY.micro, 'font-bold')}>
                <th className="px-4 py-2 text-left">Description of operating characteristics (BEHAVIOR)</th>
                <th className="px-4 py-2 text-left">Real-time audit strategy</th>
                <th className="px-4 py-2 text-left">security threat impact</th>
              </tr>
            </thead>
              ...
              {(activeNode.actions ?? ['ACL:READ', 'ACL:WRITE', 'ACL:MANAGE']).map((action) => (
                <tr key={action} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors">
                  <td className="px-4 py-2">
                    ...
                    <div className={cn(TYPOGRAPHY.legal, 'text-slate-500 dark:text-slate-400 italic mt-1')}>Register atomic business action interceptor</div>
                  </td>
