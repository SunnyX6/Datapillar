...
export function FeatureStepNavigation({
 activeNode,activeColor,ActiveIcon,targetModuleId,onTargetModuleChange,navPlacement,onNavPlacementChange,cardClassName,sectionTitleClassName
}:FeatureStepNavigationProps) {
 return (<div className="grid grid-cols-12 gap-5 animate-in slide-in-from-right-4 duration-300">
 <div className="col-span-5 space-y-4">
 <div className="space-y-2">
 <div className="px-1">
 <h3 className={sectionTitleClassName}>routing</h3>
 </div>
 ...
 <div className="space-y-2">
 <div className="px-1">
 <h3 className={sectionTitleClassName}>Parent container ownership</h3>
 </div>
 <div className="grid grid-cols-1 gap-2">
 {FEATURE_SCHEMA.map((mod) => (<Card
 key={mod.id}
 variant="default"
 padding="none"
 role="button"
 ...
 className={cn(cardClassName,'p-3 border text-left transition-all relative overflow-hidden cursor-pointer group focus:outline-none focus-visible:ring-0 shadow-none dark:shadow-none',targetModuleId === mod.id?'border-brand-400/70 bg-brand-50/30 ring-2 ring-brand-500/10 dark:border-brand-400/50 dark:bg-brand-500/10':'border-slate-100 bg-white hover:border-slate-200/80 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-slate-700')}
 >
 ...
 <p
 className={cn(TYPOGRAPHY.legal,`font-black uppercase tracking-tight truncate ${targetModuleId === mod.id?'text-brand-900 dark:text-brand-200':'text-slate-900 dark:text-slate-100'}`)}
 >
 {mod.name}
 </p>
 <p className={cn(TYPOGRAPHY.nano,'text-slate-400 dark:text-slate-500 font-medium mt-0.5')}>
 {mod.children.length} subresource
 </p>
 </div>
 </div>
 {targetModuleId === mod.id && (<div className="absolute top-0 right-0 p-2">
 ...
 <div className="space-y-2">
 <div className="px-1">
 <h3 className={sectionTitleClassName}>Mounting strategies and interactions</h3>
 </div>
 <div className="flex items-center gap-2">
 {[{ id:'global' as const,label:'global navigation' },{ id:'sidebar' as const,label:'Side navigation' }].map((item) => {
 const isActive = navPlacement === item.id
 return (<button
 key={item.id}
 type="button"
 onClick={() => onNavPlacementChange(item.id)}
 className={cn(TYPOGRAPHY.micro,'px-3 py-2 rounded-lg border font-semibold transition-colors',isActive?'border-brand-400 bg-brand-50 text-brand-700 dark:border-brand-400/60 dark:bg-brand-500/15 dark:text-brand-200':'border-slate-200 bg-white text-slate-500 hover:border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400 dark:hover:border-slate-600')}
 >
 {item.label}
 </button>)
 })}
 ...
 <h4 className={cn(TYPOGRAPHY.micro,'font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest')}>Sidebar simulation preview</h4>
 </div>
 ...
 return (<div key={mod.id} className="space-y-2">
 ...
 {mod.children.slice(0,1).map((child) => (<div
 key={child.id}
 ...
 {isTarget && (<div className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 shadow-lg shadow-slate-900/10 ring-1 ring-slate-900 dark:ring-slate-200 animate-in zoom-in-95 duration-500 origin-left scale-105">
 ...
 {mod.children.slice(1,2).map((child) => (<div
 key={child.id}
 className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 opacity-40"
 >
 <div className="w-4 h-4 rounded bg-slate-200 dark:bg-slate-700" />
