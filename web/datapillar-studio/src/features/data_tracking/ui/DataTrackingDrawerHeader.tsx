...
const headerStyleMap:Record<DrawerMode,{ bg:string;border:string;accent:string;text:string;subtitle:string }> = {
 SCHEMA:{
 bg:'bg-purple-50 dark:bg-purple-500/10',border:'border-purple-200/70 dark:border-purple-500/20',accent:'bg-purple-600 text-white',text:'text-purple-900 dark:text-purple-100',subtitle:'text-purple-600 dark:text-purple-300'
 },TRACKING:{
 bg:'bg-emerald-50 dark:bg-emerald-500/10',border:'border-emerald-200/70 dark:border-emerald-500/20',accent:'bg-emerald-600 text-white',text:'text-emerald-900 dark:text-emerald-100',subtitle:'text-emerald-600 dark:text-emerald-300'
 }
}
...
export function DataTrackingDrawerHeader({ mode,onClose }:DataTrackingDrawerHeaderProps) {
 ...
 const title = mode === 'SCHEMA'?'Define meta events (Schema)':'Add new buried points (Tracking)'
 const subtitle = mode === 'SCHEMA'?'Unified Event Model Standard,Support cross-end reuse.':'Based on 5W1H Standardize and complete buried point implementation configuration.'
 const actionLabel = mode === 'SCHEMA'?'save meta event':'Save buried points'

 return (<div className={`h-16 px-6 flex items-center justify-between border-b ${style.bg} ${style.border}`}>
