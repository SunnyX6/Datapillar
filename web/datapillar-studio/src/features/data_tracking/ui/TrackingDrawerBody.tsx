...
import {
 AlertCircle,Fingerprint,Globe,MapPin,MousePointerClick,Search,Server,Smartphone,User,X
} from 'lucide-react'
...
import type { TrackingPlatform } from '../utils/types'
...
const platformOptions:Array<{ id:TrackingPlatform;label:string;icon:typeof Globe }> = [{ id:'Web',label:'Web',icon:Globe },{ id:'App',label:'App',icon:Smartphone },{ id:'Server',label:'Server',icon:Server }]
...
export function TrackingDrawerBody() {
 ...
 const [contextProperties,setContextProperties] = useState<ContextProperty[]>([{ id:'ctx_network',name:'network_type' },{ id:'ctx_referrer',name:'referrer' }])
 const [newContextProperty,setNewContextProperty] = useState('')

 const selectedSchema = useMemo(() => EVENT_SCHEMAS.find((schema) => schema.id === selectedSchemaId),[selectedSchemaId])
 ...
 <div>
 <div className="text-body-sm font-semibold text-slate-900 dark:text-slate-100">user identity(SDK Automatic collection)</div>
 ...
 isActive?'bg-white text-emerald-700 shadow-sm dark:bg-slate-900 dark:text-emerald-300':'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'
 }`}
 >
 ...
 placeholder="Describe trigger conditions,Such as:Triggered after payment gateway bounces."
 ...
 >
 {EVENT_SCHEMAS.map((schema) => (<option key={schema.id} value={schema.id}>
 {schema.name}.{schema.key}
 </option>))}
 </select>
 </div>
 {selectedSchema && (<div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/60 p-3">
 ...
 <div className="text-caption font-semibold text-slate-500 uppercase tracking-wider">context attribute</div>
 <div className="text-micro text-slate-400 mt-1">Supplement the buried point context field,for analysis and filtering.</div>
 </div>
 ...
 {contextProperties.map((property) => (<div key={property.id} className="px-5 py-3 flex items-center justify-between">
