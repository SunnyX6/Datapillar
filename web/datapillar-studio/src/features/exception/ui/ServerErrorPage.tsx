...
export function ServerErrorPage() {
 ...
 className={cn('inline-flex items-center justify-center rounded-full border px-6 py-3 text-sm font-medium transition-all duration-300',serviceHealthState === 'healthy'?'border-emerald-300 bg-emerald-50 text-emerald-700 hover:-translate-y-0.5 hover:border-emerald-400 hover:bg-emerald-100/70 hover:shadow-lg':serviceHealthState === 'checking'?'border-amber-300 bg-amber-50 text-amber-700 cursor-wait':'border-rose-300 bg-rose-50 text-rose-600 hover:-translate-y-0.5 hover:border-rose-400 hover:bg-rose-100/70 hover:shadow-lg')}
 >
 <Activity
 className={cn('mr-2 h-4 w-4',serviceHealthState === 'healthy'?'text-emerald-500':serviceHealthState === 'checking'?'text-amber-500 animate-pulse':'text-rose-500')}
 />
 {serviceStatusLabel}
 </button>
 </div>
