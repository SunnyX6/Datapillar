...
import type { WorkflowLayoutResult } from '@/services/workflowLayoutService'
import { useComponentStore } from '@/features/workflow/state'
  ...
  useEffect(() => {
    ...
    setEdges((previous) => {
      ...
      return formattedLayout.edges.map((edge) => {
        ...
        const existingMarker = existing?.markerEnd && typeof existing.markerEnd === 'object'
          ? existing.markerEnd
          : undefined
        const sameStyle =
          existing?.animated === true &&
          existingMarker?.type === MarkerType.ArrowClosed &&
          existingMarker?.color === '#818cf8' &&
