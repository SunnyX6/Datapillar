...
import type { AgentActivity,ChatMessage } from '@/features/workflow/state'
import { getProcessRowMessage,getProcessRowTitle,getProcessStatusLabel } from './chatMessageUtils'
...
export function ChatMessageList({
 messages,forceScrollVersion,autoScrollEnabled,latestAssistantMessageId,restoreStateFrom,virtuosoRef,onAbort,onQuickSend,className
 ...
 components={{
 Scroller:ChatScroller,List:ChatList
 }}
 itemContent={(_,message) => (<div className="pb-3">
 <ChatBubble
 message={message}
...
const ChatList = forwardRef<HTMLDivElement,HTMLAttributes<HTMLDivElement>>(({ className,...props },ref) => (<div ref={ref} {...props} className={cn('flex flex-col px-4 pb-4 pt-2',className)} />))
ChatList.displayName = 'ChatList'

const ChatScroller = forwardRef<HTMLDivElement,HTMLAttributes<HTMLDivElement>>(({ className,...props },ref) => (<div ref={ref} {...props} className={cn('overflow-y-auto scrollbar-invisible',className)} />))
ChatScroller.displayName = 'ChatScroller'

const ChatBubble = memo(function ChatBubble({
 message,onQuickSend,onAbort:_onAbort,isLatestAssistant
 }:{
 message:ChatMessage
 onQuickSend?: (value:string) => void
 onAbort?: () => void
 ...
 const processRows = useMemo(() => message.processRows?? message.agentRows?? [],[message.processRows,message.agentRows])
 ...
 const latestProcessLabel = latestProcessRow?latestStatusLabel?`${buildRowTitle(latestProcessRow)}.${latestStatusLabel}`:buildRowTitle(latestProcessRow):latestStatusLabel
 ...
 return (<div className={cn('flex items-start gap-2',isUser && 'flex-row-reverse')}>
 <div
 className={cn('flex items-center justify-center flex-shrink-0',isUser?'w-8 h-8 rounded-full bg-slate-200 text-slate-600':'w-7 h-7 xl:w-8 xl:h-8 rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-600 text-white shadow-lg shadow-indigo-500/30 mt-1')}
 >
 {isUser?<User size={avatarIconSize} />:<Bot size={avatarIconSize} className="xl:scale-110" />}
 </div>
 <div
 className={cn('flex flex-col gap-1',isUser?'items-end max-w-[80%]':'items-start max-w-[90%]')}
 >
 {!isUser && (<div className="flex w-full items-center justify-between text-nano text-gray-400 dark:text-slate-400 font-medium ml-1 mb-1">
 <span>Datapillar AI • {formattedTime}</span>
 </div>)}
 <div
 className={cn(isUser?'rounded-2xl leading-relaxed border whitespace-pre-wrap bg-indigo-600 text-white rounded-tr-none border-indigo-600/60 mt-1.5 px-3 py-2 text-caption xl:text-body-sm font-normal':'bg-gray-100/80 dark:bg-slate-800/70 rounded-2xl rounded-tl-sm px-3 py-2 text-caption xl:text-body-sm font-normal text-gray-700 dark:text-slate-100 leading-relaxed border border-gray-200/50 dark:border-slate-700/60 mb-2 shadow-sm')}
 >
 {/* streaming status:Direct rendering process line */}
 {isStreaming && hasProcessRows && (<div className="space-y-2 mb-2 pb-2 border-b border-slate-100 dark:border-slate-700/50">
 <div className="px-1 flex items-center gap-2">
 <Loader2 size={10} className="animate-spin text-indigo-500" />
 <span className="text-tiny font-black text-indigo-500 uppercase tracking-widest">process dynamics</span>
 </div>
 {processRows.map((row) => (<div key={row.id} className="flex items-center gap-3">
 ...
 {/* Streaming status but no process rows:Show loading */}
 {isStreaming &&!hasProcessRows && (<div className="flex items-center gap-3 py-2">
 <div className="flex space-x-1">
 <div className="w-1.5 h-1.5 bg-indigo-500 rounded-full" />
 <div className="w-1.5 h-1.5 bg-indigo-500 rounded-full" />
 <div className="w-1.5 h-1.5 bg-indigo-500 rounded-full" />
 </div>
 <span className="text-nano font-bold text-indigo-500 tracking-tight italic">Processing</span>
 </div>)}

 {/* Non-streaming state with process lines:Foldable */}
 {!isStreaming && hasProcessRows && (<div className="mb-2 pb-2 border-b border-slate-100 dark:border-slate-700/50">
 <button
 type="button"
 ...
 <span className="text-tiny font-bold text-slate-400 uppercase tracking-widest">process record</span>
 ...
 {isExpanded && (<div className="space-y-2 pt-3">
 {processRows.map((row) => (<div key={row.id} className="flex items-center gap-3">
 <div
 ...
 {hasInterrupt && (<div className="mt-2 pt-2 border-t border-slate-100 dark:border-slate-700/50">
 ...
 {interruptSummary || 'Additional information is needed to continue.'}
 </div>
 {isLatestAssistant && _onAbort && canAbortInterrupt && (<button
 type="button"
 onClick={() => _onAbort()}
 ...
 {interrupt?.options && interrupt.options.length > 0 && (<div className="mt-2 flex flex-wrap gap-2">
 {interrupt.options.map((option) => (<button
 key={option}
 type="button"
 ...
 >
 <span className="text-tiny xl:text-nano font-normal uppercase tracking-wider text-amber-400 dark:text-amber-300">Confirm</span>
 <span>{option}</span>
 </button>))}
 </div>)}
 </div>)}
 </div>
 {shouldShowAiGuide && recommendations.length > 0 && (<div className="flex flex-wrap gap-1.5 mb-1.5">
 {recommendations.map((item) => (<button
 key={item}
 type="button"
