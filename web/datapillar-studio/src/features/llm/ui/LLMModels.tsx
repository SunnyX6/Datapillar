...
import type { LlmProvider,ModelFilters,ModelRecord } from '../utils/types'
...
export function LLMModels({
 models,isLoadingModels,filters,providerOptions,providerModelIdMap,providerBaseUrlMap,activeModelId,connectedModelIds,onModelSelect,onOpenTest,onCreateModel
}:{
 models:ModelRecord[]
 ...
 connectedModelIds:number[]
 onModelSelect:(aiModelId:number,isConnected:boolean) => void
 ...
 const selectedModels = useMemo(() => models.filter((model) => selectedModelIds.includes(model.aiModelId)),[models,selectedModelIds])

 const toggleSelection = (aiModelId:number) => {
 setSelectedModelIds((prev) => toggleModelSelection(prev,aiModelId))
 }
 ...
 viewMode === 'list'?'bg-white dark:bg-slate-700 shadow-sm text-slate-900 dark:text-white':'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'
 }`}
 >
 ...
 className={`p-1.5 rounded-md transition-all ${
 viewMode === 'grid'?'bg-white dark:bg-slate-700 shadow-sm text-slate-900 dark:text-white':'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'
 }`}
 >
 ...
 const isSelected = selectedModelIds.includes(model.aiModelId)
 ...
 isActive={activeModelId === model.aiModelId}
 isConnected={isConnected}
 onToggle={() => toggleSelection(model.aiModelId)}
 onSelect={() => onModelSelect(model.aiModelId,isConnected)}
 onManage={() => onOpenTest(model.aiModelId,isConnected?'playground':'config')}
 />)
 })}
 </div>):(<div className="grid grid-cols-1 @md:grid-cols-2 @xl:grid-cols-3 gap-4 pb-4">
 {filteredModels.map((model) => {
 const isConnected = connectedModelIds.includes(model.aiModelId) || Boolean(model.hasApiKey)
 const isSelected = selectedModelIds.includes(model.aiModelId)
 const isSelectionDisabled =!isSelected && selectedModelIds.length >= MAX_COMPARE_MODELS
 ...
 {selectedModelIds.length > 0 && (<div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 animate-in slide-in-from-bottom-4 duration-300">
...
function CompareModal({
 ...
 <div className="text-micro text-slate-400 font-mono break-all">{model.providerModelId}</div>
 </th>))}
 </tr>
 </thead>
...
function ModelRow({
 model,isSelected,isSelectionDisabled,isActive,isConnected,onToggle,onSelect,onManage
}:{
 model:ModelRecord
 isSelected:boolean
 ...
 isSelected?'bg-indigo-600 border-indigo-600 text-white cursor-pointer':isSelectionDisabled?'border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-950 text-transparent opacity-40 cursor-not-allowed':'border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-950 group-hover:border-slate-400 text-transparent cursor-pointer'
 }`}
 >
...
function ModelCard({
 ...
 isSelected?'border-indigo-400 ring-1 ring-indigo-400/30 shadow-sm':'border-slate-200 dark:border-slate-800 hover:shadow-md hover:border-slate-300 dark:hover:border-slate-700'
 } ${isActive?'ring-1 ring-indigo-200/70 dark:ring-indigo-500/30':''}`}
 onClick={onSelect}
 >
 ...
 isSelected?'bg-indigo-600 border-indigo-600 text-white cursor-pointer':isSelectionDisabled?'border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-950 text-transparent opacity-40 cursor-not-allowed':'border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-950 opacity-0 group-hover:opacity-100 hover:border-slate-400 text-transparent cursor-pointer'
 }`}
 >
 <Check size={10} strokeWidth={3} />
 </div>
...
function StatLine({ label,value,icon }:{ label:string;value:string;icon?: LucideIcon }) {
 const Icon = icon
 return (<div className="flex justify-between text-caption text-slate-500 dark:text-slate-400">
