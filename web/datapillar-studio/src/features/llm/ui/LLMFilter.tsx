...
function normalizeProviderModelIds(modelIds?: string[] | null):string[] {
 ...
 const normalized = modelIds.map((modelId) => modelId.trim()).filter((modelId) => modelId.length > 0)
...
export function LLMFilter({
 providerOptions,filterTypeOptions,contextOptions,filters,providers,onToggleProvider,onToggleType,onToggleContext,onCreateProvider,onUpdateProvider,onDeleteProvider
}:{
 ...
 setProviderCreateForm((prev) => ({...prev,modelIds:[...prev.modelIds,'']
 }))
 }

 const handleChangeCreateProviderModelId = (index:number,value:string) => {
 setProviderCreateForm((prev) => ({...prev,modelIds:prev.modelIds.map((modelId,modelIndex) => (modelIndex === index?value:modelId))
 }))
 }

 const handleRemoveCreateProviderModelId = (index:number) => {
 setProviderCreateForm((prev) => ({...prev,modelIds:prev.modelIds.filter((_,modelIndex) => modelIndex!== index)
 }))
 }

 const handleAddEditProviderModelId = () => {
 setProviderEditForm((prev) => ({...prev,modelIds:[...prev.modelIds,'']
 }))
 }

 const handleChangeEditProviderModelId = (index:number,value:string) => {
 setProviderEditForm((prev) => ({...prev,modelIds:prev.modelIds.map((modelId,modelIndex) => (modelIndex === index?value:modelId))
 }))
 }

 const handleRemoveEditProviderModelId = (index:number) => {
 setProviderEditForm((prev) => ({...prev,modelIds:prev.modelIds.filter((_,modelIndex) => modelIndex!== index)
 }))
 }
 ...
 setProviderEditForm({
 code:providerCode,name:provider.name?.trim()?? '',baseUrl:normalizeProviderBaseUrl(provider.baseUrl),modelIds:normalizeProviderModelIds(provider.modelIds)
 })
 ...
 code:providerCode,name:providerName || undefined,baseUrl:baseUrl || undefined,modelIds:createModelIds
 })
 setIsCreateProviderOpen(false)
 setProviderCreateForm(emptyProviderCreateForm)
...
function FilterSection({
 title,options,selected,onToggle,onAddOption,onEditOption,onDeleteOption,defaultOpen = false
}:{
 title:string
 ...
 return (<div key={option.value} className="group flex items-center gap-2">
