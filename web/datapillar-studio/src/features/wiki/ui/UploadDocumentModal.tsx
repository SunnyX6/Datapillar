...
export default function UploadDocumentModal({
 ...
 const [proxyAgent,setProxyAgent] = useState('Default proxy')
 ...
 {!isUploading && (<div className="flex border-b border-slate-200 dark:border-slate-800 px-8">
 <button
 type="button"
 ...
 {urlAuthEnabled && (<div className="p-4 bg-amber-50/50 rounded-xl border border-amber-100 space-y-4 animate-in fade-in slide-in-from-top-2">
 <div className="flex space-x-2 p-1 bg-white rounded-lg border border-amber-100 w-fit">
 {(['Bearer','Header','Basic'] as AuthType[]).map((type) => (<button
 key={type}
 type="button"
 onClick={() => setUrlAuthType(type)}
 className={`px-3 py-1.5 ${TYPOGRAPHY.legal} font-semibold rounded-md transition-all ${
 urlAuthType === type?'bg-amber-100 text-amber-700':'text-slate-500 hover:text-slate-700'
 }`}
 >
 {type}
 </button>))}
