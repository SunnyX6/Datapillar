    ...
    <div className="space-y-4">
      <div>
        <label className="block text-body-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
          {t('namespaceForm.field.name')} <span className="text-red-500">*</span>
        </label>
        <input
          type="text"
          value={value.name}
          onChange={handleNameChange}
          placeholder={t('namespaceForm.placeholder.name')}
        ...
        />
        {showNameError && (
          <p className="mt-1 text-micro text-rose-500">{t('namespaceForm.validation.duplicateName')}</p>
        )}
      </div>
      <div>
        <label className="block text-body-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{t('namespaceForm.field.descriptionOptional')}</label>
        <textarea
          value={value.description}
          onChange={handleDescriptionChange}
          placeholder={t('namespaceForm.placeholder.description')}
          rows={4}
