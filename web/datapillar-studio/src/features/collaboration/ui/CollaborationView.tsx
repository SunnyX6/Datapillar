...
export function CollaborationView() {
 ...
 const createTypeLabel = createType?t(`requestType.${createType}.title`, { defaultValue:requestTypeMap[createType]?.title?? createType }):''
 ...
 const baseDetails = {
 description:accessReason.trim() || genericDescription.trim(),priority:createType === 'DATA_ACCESS'?'MEDIUM':genericPriority,tags:[t('view.defaults.collaborationRequestTag')]
 }

 let detailsPayload:Ticket['details'] = {...baseDetails,target:t('view.defaults.notFilledIn')
 }

 if (createType === 'DATA_ACCESS') {
 detailsPayload = {...baseDetails,target:selectedTable?.name || t('view.defaults.noTableSelected'),permissions:[permissionType,hasPIISelected?t('view.defaults.desensitization'):t('view.defaults.originalData')],selectedColumns,duration:accessDuration
 }
 } else {
 const nextTags = normalizeTags(genericTags)
 detailsPayload = {...baseDetails,target:genericTarget.trim(),description:genericDescription.trim(),priority:genericPriority,tags:nextTags.length > 0?nextTags:[t('view.defaults.collaborationRequestTag')],expectedDate:genericExpectedDate || undefined
 }
 }

 const newTicket:Ticket = {
 id:newId,title:buildTicketTitle(t(`requestType.${createType}.title`, { defaultValue:requestTypeMap[createType]?.title?? createType }),detailsPayload.target),type:createType,status:'PENDING',createdAt:t('time.justNow'),updatedAt:t('time.justNow'),requester:currentUser,assignee:{ name:'DataOwner team',avatar:'number',role:'Asset Manager' },details:detailsPayload,timeline:[{ id:`e_${Date.now()}`,user:currentUser,action:t('timelineActions.createRequest'),time:t('time.justNow') }]
 }
 ...
 createStep === 'SELECT_TYPE'?t('view.modal.selectTypeTitle'):t('view.modal.fillFormTitle', { typeLabel:createTypeLabel })
 const createModalFooterLeft =
 createStep === 'FILL_FORM'?null:(<span className={cn(TYPOGRAPHY.caption,'text-slate-400 dark:text-slate-500')}>{t('view.modal.selectTypeHint')}</span>)
 ...
 const handleApprove = () => {
 handleStatusUpdate('APPROVED',t('timelineActions.approveRequest'))
 }

 const handleReject = () => {
 handleStatusUpdate('REJECTED',t('timelineActions.denyRequest'))
 }

 const handleCancel = () => {
 handleStatusUpdate('REJECTED',t('timelineActions.withdrawRequest'))
 }
 ...
 const nowLabel = t('time.justNow')
 const event = {
 id:`e_${Date.now()}`,user:currentUser,action:t('timelineActions.additionalInfo'),comment:trimmedComment,time:nowLabel
 }
 ...
 className="w-16 flex flex-col items-center justify-center p-2 border-2 border-dashed border-slate-200 dark:border-slate-700 hover:border-brand-300 dark:hover:border-brand-400/50 hover:bg-brand-50 dark:hover:bg-brand-900/20 text-slate-400 dark:text-slate-500 hover:text-brand-600 dark:hover:text-brand-300 transition-all flex-shrink-0 cursor-pointer group select-none"
	 ...
	 <span className="text-micro font-bold uppercase tracking-wider">{t('view.typeCarousel.more')}</span>
	 </Card>
 </div>
 ...
 className="w-16 flex flex-col items-center justify-center p-2 border-2 border-dashed border-slate-200 dark:border-slate-700 hover:border-brand-300 dark:hover:border-brand-400/50 hover:bg-brand-50 dark:hover:bg-brand-900/20 text-slate-400 dark:text-slate-500 hover:text-brand-600 dark:hover:text-brand-300 transition-all flex-shrink-0 cursor-pointer group select-none"
	 ...
	 <span className="text-micro font-bold uppercase tracking-wider">{t('view.typeCarousel.back')}</span>
	 </Card>
 ...
 <input
	 ...
	 placeholder={t('view.dataAccess.searchPlaceholder')}
 ...
 />
	 ...
	 {t('view.dataAccess.noMatch')}
	 </div>)}

	 {filteredCatalog.length > 0 && (<>
	 {!searchQuery && (<div className="px-1 mb-2 text-micro text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider">{t('view.dataAccess.recommended')}</div>)}
 ...
 >
	 ...
	 {t('view.dataAccess.tableRows', { rows:table.rows })}
	 </span>
 </div>
 ...
 {table.description}
	 ...
	 {t('view.dataAccess.ownerLabel')}:{table.owner}
	 </div>
 ...
 variant="link"
	 ...
	 >
	 {selectedColumns.length === selectedTable.columns.length?t('view.dataAccess.deselectAll'):t('view.dataAccess.selectAll')}
	 </Button>
 </div>
 ...
 <tr>
	 ...
	 <th className="px-3 py-2 font-medium text-slate-500 dark:text-slate-400">{t('view.dataAccess.tableHeader.fieldName')}</th>
	 <th className="px-3 py-2 font-medium text-slate-500 dark:text-slate-400 text-right">{t('view.dataAccess.tableHeader.type')}</th>
	 </tr>
 </thead>
 ...
 {selectedColumns.includes(column.name)?(<CheckSquare size={14} className="text-blue-600" />):(<Square size={14} className="text-slate-300 dark:text-slate-600" />)}
	 ...
	 {column.name}
	 {column.isPII && (<span title={t('view.dataAccess.sensitiveField')}>
 ...
 {column.type}
	 ...
	 <span>
	 {t('view.dataAccess.selectedColumnsPrefix')} <strong className="text-slate-900 dark:text-slate-100">{selectedColumns.length}</strong>
	 </span>
	 ...
	 <ShieldAlert size={10} className="mr-1" /> {t('view.dataAccess.containsSensitive')}
	 </span>)}
	 </div>)}
 </div>
 ...
 <div className="grid grid-cols-2 gap-4">
	 ...
	 {t('view.dataAccess.permissionType')}
	 </label>
 <select
 ...
 onChange={(event) => handlePermissionTypeChange(event.target.value)}
	 ...
	 >
	 <option value="SELECT">{t('view.dataAccess.permissionOptions.select')}</option>
	 <option value="EXPORT">{t('view.dataAccess.permissionOptions.export')}</option>
	 </select>
	 </div>
	 <div>
	 <label className={cn(TYPOGRAPHY.caption,'font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wide block mb-1.5')}>
	 {t('view.dataAccess.duration')}
	 </label>
 <select
 value={accessDuration}
	 ...
	 className={cn('w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100 focus:border-blue-500 outline-none',TYPOGRAPHY.bodySm)}
	 >
	 <option value="7 day">{t('duration.days7')}</option>
	 <option value="30 day">{t('duration.days30')}</option>
	 <option value="90 day">{t('duration.days90')}</option>
	 <option value="long term">{t('duration.longTerm')}</option>
	 </select>
	 </div>
	 </div>
	 ...
	 <div>
	 <label className={cn(TYPOGRAPHY.caption,'font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wide block mb-1.5')}>
	 {t('view.dataAccess.step3Title')}
	 </label>
 <textarea
	 value={accessReason}
	 onChange={(event) => setAccessReason(event.target.value)}
	 placeholder={t('view.dataAccess.reasonPlaceholder')}
 ...
 />
	 ...
	 <p className={TYPOGRAPHY.bodySm}>{t('view.dataAccess.selectTableFirst')}</p>
	 </div>)}
 </div>
 </div>)}
 ...
 {createStep === 'FILL_FORM' && createType && createType!== 'DATA_ACCESS' && (<div className="p-8 space-y-4">
	 <div className="grid grid-cols-2 gap-4">
		 <div>
		 <label className={cn(TYPOGRAPHY.caption,'font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wide block mb-1.5')}>
		 {t('view.generic.target')}
		 </label>
	 <input
		 value={genericTarget}
		 onChange={(event) => setGenericTarget(event.target.value)}
		 placeholder={t('view.generic.targetPlaceholder')}
	 ...
	 <input
		 ...
		 placeholder={t('view.generic.tagsPlaceholder')}
	 className={cn('w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-brand-500 outline-none',TYPOGRAPHY.bodySm)}
	 />
	 </div>
	 </div>
...
function RequestTypeCard({ type,onSelect }:{ type:RequestTypeConfig;onSelect:(typeId:TicketType) => void }) {
 ...
 <span className="text-subtitle font-bold text-slate-900 dark:text-slate-100 mb-2">{t(`requestType.${type.id}.title`, { defaultValue:type.title })}</span>
 <span className="text-caption text-slate-500 dark:text-slate-400 leading-relaxed px-2 line-clamp-3">{t(`requestType.${type.id}.desc`, { defaultValue:type.desc })}</span>
 </Card>)
}
