...
import { priorityConfigMap,requestTypeMap,statusConfigMap } from '../utils/constants'
...
export function CollaborationDetail({
 ...
 {selectedTicket.type === 'DATA_ACCESS' && (<div>
 ...
 key={column}
 className={cn('px-2 py-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded text-slate-600 dark:text-slate-300 font-mono',TYPOGRAPHY.caption)}
 >
 {column}
 </span>))}
 </div>
 ...
 {selectedTicket.details.description}
	 ...
	 {selectedTicket.type === 'CODE_REVIEW' && (<div>
 ...
 {selectedTicket.timeline.map((event) => (<div key={event.id} className="relative flex items-start group">
	 ...
	 {event.comment && (<p className={cn(TYPOGRAPHY.caption,'text-slate-600 dark:text-slate-300 mt-2 bg-slate-50 dark:bg-slate-900 p-2 rounded border border-slate-100 dark:border-slate-800')}>
	 {event.comment}
	 </p>)}
	 </div>
 </div>))}
