...
const HealthStrip = ({ status }:{ status:WorkflowDefinition['status'] }) => {
 ...
 const baseColor = isHealthy?'bg-emerald-500':isRunning?'bg-blue-500':isWarning?'bg-amber-500':isError?'bg-red-500':'bg-slate-300 dark:bg-slate-600'
...
export function StackWorkflowList({ workflows,onSelect }:StackWorkflowListProps) {
 // Simulate real paging:Dynamic pageSize(According to the current data row height + Container visible height calculation),and press offset/limit Append batch by batch.
 ...
 return (<div ref={containerRef} className="flex-1 overflow-auto custom-scrollbar p-6">
 ...
 {workflow.tags.includes('P0') && (<span className="ml-2 px-1.5 py-0.5 bg-red-50 text-red-600 text-nano font-bold rounded border border-red-100 dark:bg-red-500/10 dark:text-red-200 dark:border-red-500/20">
 P0
 </span>)}
 </div>
