...
test.describe('project module', () => {
  test('Create a project and enter the offline data warehouse stack', async ({ page }) => {
    await login(page)
    ...
    await projectCard.getByText('Offline data warehouse', { exact: true }).click()

    await expect(page.getByText(projectName)).toBeVisible()
    await expect(page.getByText('Overview')).toBeVisible()
  })

  test('The task management canvas is centered and expanded to the maximum height by default at the bottom', async ({ page }) => {
    ...
    if (!scrollBox || !graphBox) {
      throw new Error('Canvas container positioning failed')
    }
    ...
    if (!bottomBox || !mainBox) {
      throw new Error('Bottom panel positioning failed')
    }
