...
```python
# 注册公司知识
KnowledgeRegistry.register_domain(data_analysis_methodology)

# Agent 入职时注册知识
KnowledgeRegistry.register_agent_knowledge("analyst", [my_contribution])

# 按需获取知识
knowledge = KnowledgeRegistry.get_knowledge(
...
class KnowledgeRegistry:
    ...
    def get_knowledge_prompt(
        ...
        result = cls.get_knowledge(domains, agent_id, max_tokens)
        domain_contents = result.get("domains", {})

        if not domain_contents:
            return ""

        parts = ["## 知识上下文\n"]
        for content in domain_contents.values():
