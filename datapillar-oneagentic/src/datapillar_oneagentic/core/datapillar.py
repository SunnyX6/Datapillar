...
class DatapillarResult:
    ...
    # 获取特定 Agent 的输出（用 deliverable_key）
    analysis = result.get_deliverable("analysis")
    code = result.get_deliverable("code")

    # 或直接访问
    result.deliverables["analysis"]
    ...
    """各 Agent 交付物 {deliverable_key: deliverable}"""
...
class Datapillar:
    ...
    def __init__(
        ...
        *,
        ...
        - checkpointer: Checkpointer 实例（状态持久化，不传则用内存）
        - deliverable_store: Store 实例（交付物存储，不传则用内存）
        - learning_store: VectorStore 实例（经验学习存储，不传则用 LanceDB）
        - enable_memory: 是否启用对话记忆（默认 True）
        ...
        from datapillar_oneagentic.storage import RedisCheckpointer, PostgresStore
        ...
        deliverable_store = PostgresStore(url="postgresql://...")
        ...
        logger.info(
            f"Datapillar 创建: {name} ({self.team_id}), "
    ...
    def _route_by_active_agent(self, state) -> str:
        """根据 active_agent 路由"""
    ...
    def _route_after_agent(self, state) -> str:
        """Agent 执行后的路由"""
        active = state.get("active_agent")
        if active and active in self._agent_ids:
            return active
