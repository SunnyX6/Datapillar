...
class Compactor:
    ...
    async def compress(
        ...
        if existing_summary:
            tokens_before += self._token_counter.count(existing_summary)

        # 检查是否需要压缩
        if tokens_before <= self.policy.get_trigger_tokens():
        ...
        # 计算压缩后 token 数
        tokens_after = self._token_counter.count(summary)
    ...
    def _estimate_entries_tokens(self, entries: list[ConversationEntry]) -> int:
        ...
        total = 0
        for entry in entries:
            total += self._token_counter.count(entry.content) + 30
        return total
