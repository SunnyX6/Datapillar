...
async def test_save_success() -> None:
    ...
    learner.start_recording("s1", "goal 1")
    learner.record_tool("s1", "tool_a")
    learner.record_agent("s1", "agent_a")
    learner.complete_recording("s1", "success", result_summary="completed")
...
@pytest.mark.asyncio
async def test_save_failure() -> None:
    store = _StubExperienceStore()
...
@pytest.mark.asyncio
async def test_save_pending() -> None:
    store = _StubExperienceStore()
    embedder = _StubEmbeddingProvider()
    learner = ExperienceLearner(store=store, namespace="ns", embedding_provider=embedder)
...
@pytest.mark.asyncio
async def test_build_context() -> None:
    ...
    context = await retriever.build_context("goal", k=2, prefer_success=True)
...
@pytest.mark.asyncio
async def test_experience_retriever() -> None:
    store = _StubExperienceStore()
    embedder = _StubEmbeddingProvider(fail=True)
    retriever = ExperienceRetriever(store=store, embedding_provider=embedder)

    records = await retriever.search("goal", k=3)
    assert records == []


@pytest.mark.asyncio
async def test_experience_retriever2() -> None:
    ...
    tools = await retriever.get_common_tools("goal", k=2)
