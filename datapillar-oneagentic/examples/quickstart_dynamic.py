...
# @date 2026-01-27
"""
Datapillar OneAgentic dynamic mode example.
...
@agent(
    ...
    description="Decide whether delegation is needed",
)
class ManagerAgent:
    SYSTEM_PROMPT = """You are the manager.

Requirements:
1. You must call delegate_to_worker to delegate tasks to the worker.
2. If delegation is not possible, return the final result.

## Output requirements
Return JSON only (single object), no explanations or Markdown:
{"text": "Reason delegation is not possible"}
"""
...
@agent(
    ...
    deliverable_schema=TextOutput,
    tools=[echo],
    description="Execute the task and return results",
)
class WorkerAgent:
    SYSTEM_PROMPT = """You are the worker.
Use the echo tool to process the user request and respond.
...
async def main() -> None:
    ...
    print("=" * 60)
    print("Dynamic mode example is ready")
    print(f"  Model: {config.llm.model}")
    print("  Members: Manager -> Worker (dynamic delegation)")
    print("=" * 60)

    query = "Write a short project status update."
    print(f"\nUser request: {query}\n")
    print("-" * 60)
