...
- **Token 绑定租户**：JWT 必须包含 tenant_id；访问其他租户必须重新签发 token。
- **用户与租户关系独立**：用户可跨租户，`tenant_users` 作为成员关系与会话载体。
- **组织结构为产品内模型**：当前设计不依赖外部 OA 同步。
- **RBAC 租户化**：角色与授权均按租户分配；tenant_id=0 为系统租户，不代表全局共享。
...
## 数据模型（DDL）
- 统一 DDL：`docs/db/datapillar_schema.sql`
- 关键表：
  - 租户：`tenants`
  - 用户：`users`（全局身份，tenant_id 为主租户）
...
- RBAC：`roles/user_roles/role_permissions/user_permission_overrides/feature_objects/...`
  - 业务表：`projects/job_workflow/job_info/job_dependency/...` 均含 tenant_id

## 权限动作与租户授权（细节）
### 1) 权限动作固定（Permission）
- **权限动作是系统静态字典**：`permissions` 仅由平台维护，租户不可自定义。
- **角色可自定义**：租户管理员可创建 `roles` 并分配权限。
- `permissions.level` 表示权限等级，数值越大权限越高；`sort` 仅用于展示排序。
- **示例等级**（可扩展）：`READ=1`、`WRITE=2`、`ADMIN=3`。

### 2) 租户功能授权（上限模型）
新增表：
- `tenant_feature_permissions`：租户对功能对象的授权上限（含 permission_id）。
- `tenant_feature_audit`：授权变更审计。

核心语义：
- **租户是否可用某功能** = 是否存在 `tenant_feature_permissions` 且 `status=1`。
- **租户对功能的最高权限** = `tenant_feature_permissions.permission_id`。
- **用户最终权限** = `min(用户角色权限level, 租户功能权限level)`。
- 若租户未授权该功能或 `status=0`，则**无权限**。

### 3) 审计规则
- 任何授权变更必须写 `tenant_feature_audit`：
  - `action`: `GRANT/REVOKE/ENABLE/DISABLE/UPDATE_PERMISSION`
  - `before/after_status`
  - `before/after_permission_id`
  - `operator_user_id`、`operator_tenant_id`

### 4) 目录表归属（当前策略）
- `feature_object_categories / feature_objects / permissions` **当前仅放 tenant_id=0**（系统静态目录）。
- **租户可用范围**由 `tenant_feature_permissions` 决定。
- 未来若需要“租户自定义目录”，可新增 `tenant_id=租户` 的记录并扩展查询范围。

### 5) 强制校验点（必须落地）
- 功能/菜单列表必须过滤 `tenant_feature_permissions.status=1`。
- 角色权限分配时必须校验：`role_permissions.permission_id.level <= tenant_feature_permissions.permission_id.level`。
- 用户权限计算时必须做“租户上限收敛”。

### 6) 授权默认策略（已定）
- 采用 **B. 默认全量 READ**：新租户创建时自动授予全部功能，permission=READ。
- 不提供“初始化授权接口”，由租户创建流程/脚本写入 `tenant_feature_permissions`。
...
- `feature_objects/feature_object_categories/permissions` 仅按当前租户过滤；tenant_id=0 为系统租户数据，不再全局共享。
...
## 平台超管跨租户（已定）
- 平台超管仅在 **系统租户（tenant_id=0）** 登录。
- 判定标准：`tenant_id=0` 且角色类型为 `ADMIN`。
- **只保留一套路由**：现有业务接口路径保持不变，不新增前缀、不额外参数。
- 平台超管通过 **授权接口签发目标租户 access token（带审计）**，该 token **就是**访问目标租户业务接口的凭证，用于隔离与审计，不新增业务接口路径：
  - `POST /auth/tenants/{tenantId}/assume`
  - 校验：`tenant_id=0` 且角色 `ADMIN`
  - 返回：`tenant_id=目标租户` 的新 access token
  - 审计：token 内必须包含 `actorUserId`、`actorTenantId=0`、`impersonation=true`
- 目标租户隔离：所有数据访问按 **目标租户 token 的 tenant_id** 作为过滤条件。
- 平台超管不要求存在 `tenant_users` 成员关系。
...
**示例**
- 普通用户：`GET /users`（token.tenant_id=本租户）
- 平台超管：先 `POST /auth/tenants/23/assume`，再 `GET /users`（token.tenant_id=23）

### Assume 接口契约（平台超管授权访问目标租户）
**接口**
- `POST /auth/tenants/{tenantId}/assume`
- 仅支持 access token 鉴权（`Authorization: Bearer` 或 `Cookie: auth-token`）
- 请求体：无

**校验规则**
- access token 合法且未过期，`tokenType=access`
- `token.tenantId=0` 且用户启用
- 用户在系统租户拥有 `ADMIN` 角色
- 目标租户存在且启用

**响应**
- `Set-Cookie: auth-token=<new access>`（仅下发 access token，不下发 refresh）
- `ApiResponse<AuthDto.LoginResponse>`（字段与登录一致：`userId/tenantId/username/email/roles/menus`）

**新 token 必含 claims（审计）**
- `tenantId=目标租户`
- `actorUserId=超管userId`
- `actorTenantId=0`
- `impersonation=true`
