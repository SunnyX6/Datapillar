...
/**
 * initialization stateCheckercomponents Responsible for initializing the stateCheckerCore logic
 * implementation
 *
 * @author Sunny
 * @date 2026-01-01
...
public class SetupStateChecker {
  ...
  public SetupStateChecker(
      ReactiveDiscoveryClient discoveryClient, WebClient.Builder webClientBuilder) {
  ...
  public Mono<SetupState> currentState() {
    ...
    synchronized (this) {
      ...
      if (inFlight == null) {
        inFlight =
            fetchStateFromStudio()
                .onErrorResume(
                    ex -> {
                      log.error("setup Status probe request failed", ex);
  ...
  private Mono<SetupState> fetchStateFromStudio() {
    return discoveryClient
        .getInstances(STUDIO_SERVICE_ID)
        .next()
        .switchIfEmpty(Mono.error(new IllegalStateException("not found studio-service Example")))
  ...
  private Mono<SetupState> requestStatus(ServiceInstance instance) {
    URI uri =
        UriComponentsBuilder.fromUri(instance.getUri()).path(STATUS_ENDPOINT).build(true).toUri();
  ...
  private SetupState parseStatusResponse(Map<String, Object> response) {
    if (response == null) {
      throw new IllegalStateException("setup Status response is empty");
    }

    Integer code = parseCode(response.get("code"));
    if (code == null) {
      throw new IllegalStateException("setup Status response missing code Field");
    }

    if (code == Code.OK) {
      Object dataObject = response.get("data");
      if (!(dataObject instanceof Map<?, ?> dataMap)) {
        throw new IllegalStateException("setup Status response missing data Field");
      }
    ...
    throw new IllegalStateException("setup status response code Not OK: " + code);
  }
