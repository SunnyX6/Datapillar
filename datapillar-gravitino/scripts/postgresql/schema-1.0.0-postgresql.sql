...
CREATE TABLE IF NOT EXISTS metalake_meta (
    ...
    UNIQUE (tenant_id, metalake_name, deleted_at)
    );
...
CREATE TABLE IF NOT EXISTS catalog_meta (
    ...
    UNIQUE (tenant_id, metalake_id, catalog_name, deleted_at)
    );
...
CREATE TABLE IF NOT EXISTS schema_meta (
    ...
    UNIQUE (tenant_id, catalog_id, schema_name, deleted_at)
    );

CREATE INDEX IF NOT EXISTS schema_meta_idx_metalake_id ON schema_meta (tenant_id, metalake_id);
...
CREATE TABLE IF NOT EXISTS table_meta (
    ...
    UNIQUE (tenant_id, schema_id, table_name, deleted_at)
    );

CREATE INDEX IF NOT EXISTS table_meta_idx_metalake_id ON table_meta (tenant_id, metalake_id);
CREATE INDEX IF NOT EXISTS table_meta_idx_catalog_id ON table_meta (tenant_id, catalog_id);
...
CREATE TABLE IF NOT EXISTS table_column_version_info (
    ...
    audit_info TEXT NOT NULL,
    PRIMARY KEY (id),
    UNIQUE (tenant_id, table_id, table_version, column_id, deleted_at)
);
CREATE INDEX table_column_version_info_idx_mid ON table_column_version_info (tenant_id, metalake_id);
CREATE INDEX table_column_version_info_idx_cid ON table_column_version_info (tenant_id, catalog_id);
CREATE INDEX table_column_version_info_idx_sid ON table_column_version_info (tenant_id, schema_id);
...
CREATE TABLE IF NOT EXISTS fileset_meta (
    ...
    UNIQUE (tenant_id, schema_id, fileset_name, deleted_at)
    );

CREATE INDEX IF NOT EXISTS fileset_meta_idx_metalake_id ON fileset_meta (tenant_id, metalake_id);
CREATE INDEX IF NOT EXISTS fileset_meta_idx_catalog_id ON fileset_meta (tenant_id, catalog_id);
...
CREATE TABLE IF NOT EXISTS fileset_version_info (
    ...
    deleted_at BIGINT NOT NULL DEFAULT 0,
    PRIMARY KEY (id),
    UNIQUE (tenant_id, fileset_id, version, storage_location_name, deleted_at)
    );

CREATE INDEX IF NOT EXISTS fileset_version_info_idx_metalake_id ON fileset_version_info (tenant_id, metalake_id);
CREATE INDEX IF NOT EXISTS fileset_version_info_idx_idx_catalog_id ON fileset_version_info (tenant_id, catalog_id);
CREATE INDEX IF NOT EXISTS fileset_version_info_idx_idx_schema_id ON fileset_version_info (tenant_id, schema_id);
...
CREATE TABLE IF NOT EXISTS topic_meta (
    ...
    UNIQUE (tenant_id, schema_id, topic_name, deleted_at)
    );

CREATE INDEX IF NOT EXISTS topic_meta_idx_metalake_id ON topic_meta (tenant_id, metalake_id);
CREATE INDEX IF NOT EXISTS topic_meta_idx_catalog_id ON topic_meta (tenant_id, catalog_id);
...
CREATE TABLE IF NOT EXISTS role_meta (
    ...
    deleted_at BIGINT NOT NULL DEFAULT 0,
    PRIMARY KEY (role_id),
    UNIQUE (tenant_id, metalake_id, role_name, deleted_at)
    );
...
CREATE INDEX IF NOT EXISTS role_meta_securable_object_idx_role_id ON role_meta_securable_object (tenant_id, role_id);
...
CREATE TABLE IF NOT EXISTS user_role_rel (
    ...
    deleted_at BIGINT NOT NULL DEFAULT 0,
    PRIMARY KEY (id),
    UNIQUE (tenant_id, user_id, role_id, deleted_at)
    );

CREATE INDEX IF NOT EXISTS user_role_rel_idx_user_id ON user_role_rel (tenant_id, user_id);
...
CREATE TABLE IF NOT EXISTS group_meta (
    ...
    UNIQUE (tenant_id, metalake_id, group_name, deleted_at)
    );
...
CREATE TABLE IF NOT EXISTS group_role_rel (
    ...
    deleted_at BIGINT NOT NULL DEFAULT 0,
    PRIMARY KEY (id),
    UNIQUE (tenant_id, group_id, role_id, deleted_at)
    );

CREATE INDEX IF NOT EXISTS group_role_rel_idx_group_id ON group_role_rel (tenant_id, group_id);
...
CREATE TABLE IF NOT EXISTS tag_meta (
    ...
    deleted_at BIGINT NOT NULL DEFAULT 0,
    PRIMARY KEY (tag_id),
    UNIQUE (tenant_id, metalake_id, tag_name, deleted_at)
    );
...
CREATE TABLE IF NOT EXISTS tag_relation_meta (
    ...
    deleted_at BIGINT NOT NULL DEFAULT 0,
    PRIMARY KEY (id),
    UNIQUE (tenant_id, tag_id, metadata_object_id, metadata_object_type, deleted_at)
    );

CREATE INDEX IF NOT EXISTS tag_relation_meta_idx_tag_id ON tag_relation_meta (tenant_id, tag_id);
CREATE INDEX IF NOT EXISTS tag_relation_meta_idx_metadata_object_id ON tag_relation_meta (tenant_id, metadata_object_id);
...
CREATE TABLE IF NOT EXISTS owner_meta (
    ...
    deleted_at BIGINT NOT NULL DEFAULT 0,
    PRIMARY KEY (id),
    UNIQUE (tenant_id, owner_id, metadata_object_id, metadata_object_type, deleted_at)
    );

CREATE INDEX IF NOT EXISTS owner_meta_idx_owner_id ON owner_meta (tenant_id, owner_id);
CREATE INDEX IF NOT EXISTS owner_meta_idx_metadata_object_id ON owner_meta (tenant_id, metadata_object_id);
...
CREATE TABLE IF NOT EXISTS model_meta (
    ...
    UNIQUE (tenant_id, schema_id, model_name, deleted_at)
    );

CREATE INDEX IF NOT EXISTS model_meta_idx_metalake_id ON model_meta (tenant_id, metalake_id);
CREATE INDEX IF NOT EXISTS model_meta_idx_catalog_id ON model_meta (tenant_id, catalog_id);
...
CREATE TABLE IF NOT EXISTS model_version_info (
    ...
    deleted_at BIGINT NOT NULL DEFAULT 0,
    PRIMARY KEY (id),
    UNIQUE (tenant_id, model_id, version, model_version_uri_name, deleted_at)
    );

CREATE INDEX IF NOT EXISTS model_version_info_idx_metalake_id ON model_version_info (tenant_id, metalake_id);
CREATE INDEX IF NOT EXISTS model_version_info_idx_catalog_id ON model_version_info (tenant_id, catalog_id);
CREATE INDEX IF NOT EXISTS model_version_info_idx_schema_id ON model_version_info (tenant_id, schema_id);
...
CREATE TABLE IF NOT EXISTS model_version_alias_rel (
    ...
    deleted_at BIGINT NOT NULL DEFAULT 0,
    PRIMARY KEY (id),
    UNIQUE (tenant_id, model_id, model_version_alias, deleted_at)
    );

CREATE INDEX IF NOT EXISTS model_version_alias_rel_idx_model_version_alias on model_version_alias_rel (tenant_id, model_version_alias);
...
CREATE TABLE IF NOT EXISTS policy_meta (
    ...
    UNIQUE (tenant_id, metalake_id, policy_name, deleted_at)
);
...
CREATE TABLE IF NOT EXISTS policy_version_info (
    ...
    deleted_at BIGINT NOT NULL DEFAULT 0,
    PRIMARY KEY (id),
    UNIQUE (tenant_id, policy_id, version, deleted_at)
);

CREATE INDEX IF NOT EXISTS policy_version_info_idx_metalake_id ON policy_version_info (tenant_id, metalake_id);
...
CREATE TABLE IF NOT EXISTS policy_relation_meta (
    ...
    deleted_at BIGINT NOT NULL DEFAULT 0,
    PRIMARY KEY (id),
    UNIQUE (tenant_id, policy_id, metadata_object_id, metadata_object_type, deleted_at)
);

CREATE INDEX IF NOT EXISTS policy_relation_meta_idx_policy_id ON policy_relation_meta (tenant_id, policy_id);
CREATE INDEX IF NOT EXISTS policy_relation_meta_idx_metadata_object_id ON policy_relation_meta (tenant_id, metadata_object_id);
...
CREATE TABLE IF NOT EXISTS statistic_meta (
    ...
    UNIQUE (tenant_id, statistic_name, metadata_object_id, deleted_at)
);

CREATE INDEX IF NOT EXISTS statistic_meta_idx_stid ON statistic_meta (tenant_id, statistic_id);
CREATE INDEX IF NOT EXISTS statistic_meta_idx_moid ON statistic_meta (tenant_id, metadata_object_id);
...
CREATE TABLE IF NOT EXISTS job_template_meta (
    ...
    UNIQUE (tenant_id, metalake_id, job_template_name, deleted_at)
);
...
CREATE TABLE IF NOT EXISTS job_run_meta (
    ...
    UNIQUE (tenant_id, metalake_id, job_execution_id, deleted_at)
);

CREATE INDEX IF NOT EXISTS job_run_meta_idx_job_template_id ON job_run_meta (tenant_id, job_template_id);
CREATE INDEX IF NOT EXISTS job_run_meta_idx_job_execution_id ON job_run_meta (tenant_id, job_execution_id);
...
CREATE TABLE IF NOT EXISTS wordroot_meta (
    ...
    catalog_id BIGINT NOT NULL,
    schema_id BIGINT NOT NULL,
    root_comment TEXT DEFAULT NULL,
    audit_info TEXT NOT NULL,
    deleted_at BIGINT NOT NULL DEFAULT 0,
    PRIMARY KEY (root_id),
    UNIQUE (tenant_id, schema_id, root_code, deleted_at)
);
...
CREATE TABLE IF NOT EXISTS unit_meta (
    ...
    catalog_id BIGINT NOT NULL,
    schema_id BIGINT NOT NULL,
    unit_comment TEXT DEFAULT NULL,
    audit_info TEXT NOT NULL,
    deleted_at BIGINT NOT NULL DEFAULT 0,
    PRIMARY KEY (unit_id),
    UNIQUE (tenant_id, schema_id, unit_code, deleted_at)
);
...
COMMENT ON COLUMN unit_meta.unit_comment IS 'unit comment';
COMMENT ON COLUMN unit_meta.audit_info IS 'unit audit info';
COMMENT ON COLUMN unit_meta.deleted_at IS 'unit deleted at';
...
CREATE TABLE IF NOT EXISTS value_domain_meta (
    ...
    catalog_id BIGINT NOT NULL,
    schema_id BIGINT NOT NULL,
    domain_comment TEXT DEFAULT NULL,
    audit_info TEXT NOT NULL,
    deleted_at BIGINT NOT NULL DEFAULT 0,
    PRIMARY KEY (item_id),
    UNIQUE (tenant_id, schema_id, domain_code, item_value, deleted_at)
);
...
COMMENT ON COLUMN value_domain_meta.domain_comment IS 'domain comment';
COMMENT ON COLUMN value_domain_meta.audit_info IS 'audit info';
COMMENT ON COLUMN value_domain_meta.deleted_at IS 'deleted at';
...
CREATE TABLE IF NOT EXISTS metric_meta (
    ...
    UNIQUE (tenant_id, schema_id, metric_code, deleted_at)
);
...
CREATE TABLE IF NOT EXISTS metric_version_info (
    ...
    deleted_at BIGINT NOT NULL DEFAULT 0,
    PRIMARY KEY (id),
    UNIQUE (tenant_id, metric_id, version, deleted_at)
);

CREATE INDEX IF NOT EXISTS metric_version_info_idx_metric_id ON metric_version_info (tenant_id, metric_id);
CREATE INDEX IF NOT EXISTS metric_version_info_idx_metalake ON metric_version_info (tenant_id, metalake_id);
