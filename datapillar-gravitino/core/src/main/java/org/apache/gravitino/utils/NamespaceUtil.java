...
public class NamespaceUtil {
  ...
  /**
   * Create a namespace for metric.
   *
   * @param metalake The metalake name
   * @param catalog The catalog name
   * @param schema The schema name
   * @return A namespace for metric
   */
  public static Namespace ofMetric(String metalake, String catalog, String schema) {
    return Namespace.of(metalake, catalog, schema);
  }

  /**
   * Create a namespace for metric version.
   *
   * @param metalake The metalake name
   * @param catalog The catalog name
   * @param schema The schema name
   * @param metric The metric name
   * @return A namespace for metric version
   */
  public static Namespace ofMetricVersion(
      String metalake, String catalog, String schema, String metric) {
    return Namespace.of(metalake, catalog, schema, metric);
  }
  ...
  /**
   * Convert a metric name identifier to a metric version namespace.
   *
   * @param metricIdent The metric name identifier
   * @return A metric version namespace
   */
  public static Namespace toMetricVersionNs(NameIdentifier metricIdent) {
    return ofMetricVersion(
        metricIdent.namespace().level(0),
        metricIdent.namespace().level(1),
        metricIdent.namespace().level(2),
        metricIdent.name());
  }
  ...
  /**
   * Check if the given metric namespace is legal, throw an {@link IllegalNamespaceException} if
   * it's illegal.
   *
   * @param namespace The metric namespace
   */
  public static void checkMetric(Namespace namespace) {
    check(
        namespace != null && namespace.length() == 3,
        "Metric namespace must be non-null and have 3 levels, the input namespace is %s",
        namespace);
  }

  /**
   * Check if the given metric version namespace is legal, throw an {@link
   * IllegalNamespaceException} if it's illegal.
   *
   * @param namespace The metric version namespace
   */
  public static void checkMetricVersion(Namespace namespace) {
    check(
        namespace != null && namespace.length() == 4,
        "Metric version namespace must be non-null and have 4 levels, the input namespace is %s",
        namespace);
  }

  /**
   * Check if the given modifier namespace is legal, throw an {@link IllegalNamespaceException} if
   * it's illegal.
   *
   * @param namespace The modifier namespace
   */
  public static void checkModifier(Namespace namespace) {
    check(
        namespace != null && namespace.length() == 3,
        "Modifier namespace must be non-null and have 3 levels, the input namespace is %s",
        namespace);
  }

  /**
   * Check if the given root namespace is legal, throw an {@link IllegalNamespaceException} if it's
   * illegal.
   *
   * @param namespace The root namespace
   */
  public static void checkRoot(Namespace namespace) {
    check(
        namespace != null && namespace.length() == 3,
        "Root namespace must be non-null and have 3 levels, the input namespace is %s",
        namespace);
  }
