...
/** MetricCatalog 接口定义了在 schema 中管理指标对象的公共 API。 如果 catalog 实现支持指标对象，则应实现此接口。 */
@Evolving
public interface MetricCatalog {
   ...
   * @throws NoSuchSchemaException 如果 schema 不存在
   */
  NameIdentifier[] listMetricRoots(Namespace namespace) throws NoSuchSchemaException;

  /**
   * 通过 {@link NameIdentifier} 从 catalog 获取指标词根
   *
   * @param ident 指标词根标识符
   * @return 指标词根对象
   */
  MetricRoot getMetricRoot(NameIdentifier ident);

  /**
   * 在 catalog 中创建指标词根
   *
   * @param ident 指标词根的名称标识符
   * @param code 词根编码
   * @param nameCn 中文名称
   * @param nameEn 英文名称
   ...
   */
  boolean deleteMetricRoot(NameIdentifier ident);
}
