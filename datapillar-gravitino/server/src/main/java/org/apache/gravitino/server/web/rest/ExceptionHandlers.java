...
public class ExceptionHandlers {
  ...
  public static Response handleMetricException(
      OperationType op, String metric, String schema, Exception e) {
  ...
  private static class MetricExceptionHandler extends BaseExceptionHandler {
    private static final ExceptionHandler INSTANCE = new MetricExceptionHandler();
    ...
    @Override
    public Response handle(OperationType op, String metric, String schema, Exception e) {
      String formatted = StringUtil.isBlank(metric) ? "" : " [" + metric + "]";
      String errorMsg = getMetricErrorMsg(formatted, op.name(), schema, getErrorMsg(e));
      LOG.warn(errorMsg, e);
      ...
      } else if (e instanceof org.apache.gravitino.exceptions.MetricAlreadyExistsException) {
        return Utils.alreadyExists(errorMsg, e);
