...
plugins {
  ...
  // Spotless version >= 6.19.0 supports JDK 21
  if (JavaVersion.current() == JavaVersion.VERSION_17 || JavaVersion.current() == JavaVersion.VERSION_21) {
    ...
    throw GradleException(
      "The Gravitino Gradle toolchain currently does not support " +
        "Java version ${JavaVersion.current()}. Please use JDK version 17 or 21.",
    )
  }
...
allprojects {
  ...
  plugins.withType<com.diffplug.gradle.spotless.SpotlessPlugin>().configureEach {
    configure<com.diffplug.gradle.spotless.SpotlessExtension> {
      java {
        // Updated Google Java Format to 1.22.0 for JDK 21 compatibility
        // JDK 21 requires google-java-format >= 1.17.0
...
subprojects {
  ...
  java {
    toolchain {
      // Updated to support JDK 21
      // Some JDK vendors like Homebrew installed OpenJDK have problems in building trino-connector:
      // It will cause tests of Trino-connector hanging forever on macOS, to avoid this issue and
      // other vendor-related problems, Gravitino will use the specified AMAZON OpenJDK to build
