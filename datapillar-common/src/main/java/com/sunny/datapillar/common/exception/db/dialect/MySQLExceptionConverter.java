...
public class MySQLExceptionConverter implements SQLExceptionConverter {
  ...
  private static final Set<Integer> CONNECTION_ERROR_CODES =
      Set.of(1040, 1042, 1043, 1047, 1158, 1159, 1160, 1161, 2002, 2003, 2006, 2013);
    ...
    if (errorCode != null) {
      switch (errorCode) {
        case ERROR_DUPLICATE_KEY:
          return new DbUniqueConstraintViolationException(
              sqlException, errorCode, sqlState, constraintName);
        ...
        case ERROR_CHECK_CONSTRAINT:
          return new DbCheckConstraintViolationException(
              sqlException, errorCode, sqlState, constraintName);
