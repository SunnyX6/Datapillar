...
/**
 * SecretCodeccomponents responsibleSecretCodecCore logic implementation
 *
 * @author Sunny
 * @date 2026-01-01
 */
public final class SecretCodec {
  ...
  public static String encrypt(String publicKeyPem, String plaintext) {
    if (!hasText(publicKeyPem)) {
      throw new IllegalArgumentException("public_key_pem cannot be empty");
    }
    ...
    try {
      ...
      byte[] merged =
          ByteBuffer.allocate(encryptedAesKey.length + nonce.length + encryptedPayload.length)
              .put(encryptedAesKey)
              .put(nonce)
      ...
      throw new IllegalArgumentException("secret Encryption failed", ex);
    }
  }
  ...
  public static String decrypt(byte[] privateKeyPem, String encoded) {
    ...
    if (!hasText(privateKeyPem)) {
      throw new IllegalArgumentException("private_key_pem cannot be empty");
    }
    if (!encoded.startsWith(ENC_PREFIX)) {
      throw new IllegalArgumentException("secret Not encrypted");
    }
    try {
      ...
      byte[] nonce =
          Arrays.copyOfRange(payload, encryptedKeyLength, encryptedKeyLength + GCM_NONCE_BYTES);
      byte[] encryptedPayload =
          Arrays.copyOfRange(payload, encryptedKeyLength + GCM_NONCE_BYTES, payload.length);
      ...
      throw new IllegalArgumentException("secret Decryption failed", ex);
    }
  }
  ...
  private static byte[] encryptWithAesGcm(SecretKey aesKey, byte[] nonce, String plaintext)
      throws Exception {
  ...
  private static String decryptWithAesGcm(SecretKey aesKey, byte[] nonce, byte[] ciphertext)
      throws Exception {
    Cipher cipher = Cipher.getInstance("AES/GCM/NoPadding");
    GCMParameterSpec spec = new GCMParameterSpec(GCM_TAG_BITS, nonce);
  ...
  private static byte[] decryptAesKey(PrivateKey privateKey, byte[] encryptedAesKey)
      throws Exception {
    Cipher rsaCipher = Cipher.getInstance("RSA/ECB/OAEPWithSHA-256AndMGF1Padding");
