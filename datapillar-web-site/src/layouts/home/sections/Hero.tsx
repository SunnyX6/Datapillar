...
const useTypingText = (text: string, isActive: boolean, speed: number) => {
...
export function Hero({ onRequestAccess }: HeroProps) {
  ...
  return (
    <section id="studio" className="relative pt-40 pb-24 overflow-hidden border-b border-white/5 bg-[#020410]">
      ...
      <div className="relative max-w-7xl mx-auto px-8 flex flex-col items-center text-center z-10">
        ...
        <div className="relative w-full max-w-6xl mx-auto perspective-1000 group mb-8">
          ...
          >
            ...
            {activeScenario === 0 ? <PipelineView key="pipeline" /> : <AnalyticsView key="analytics" />}
          </div>
        </div>
...
const PipelineView = () => {
  ...
  const typedPrompt = useTypingText(prompt, isPendingInput, typingSpeed)
            ...
            <div className={`flex-1 text-micro font-mono ${isPendingInput ? 'text-slate-200' : 'text-slate-500'}`}>
              {isPendingInput ? (
                <span className="flex items-center min-h-[1rem]">
                  <span>{typedPrompt.length ? typedPrompt : '\u200B'}</span>
      ...
      <div className="flex-1 relative bg-[#0b0f19] overflow-hidden">
        ...
        <div className="absolute inset-0 z-0 flex justify-around items-center overflow-hidden transition-opacity duration-700" style={{ opacity: matrixOpacity }}>
          {matrixColumns.map((column, colIdx) => (
            <div key={colIdx} className="text-[10px] text-indigo-400 font-mono whitespace-pre flex flex-col gap-3">
        ...
        <div className="relative z-10 w-full h-full flex items-center justify-center">
          <div className="relative" style={{ width: `${centerWidth}px`, height: `${centerHeight}px` }}>
            <svg
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="relative" style={{ width: `${previewWidth}px`, height: `${previewHeight}px` }}>
                {canvasNodes.map((node, index) => {
                  ...
                  return (
                    <div
                      ...
                      className={`absolute rounded-xl border bg-[#0f172a] flex flex-col items-center justify-center gap-1 transition-all duration-300 z-30 ${
