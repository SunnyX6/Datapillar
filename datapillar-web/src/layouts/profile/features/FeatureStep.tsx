...
import type { PermissionLevel, RoleItem } from '../permission/Permission'
import { FEATURE_SCHEMA, type FeatureModule, type FeatureNode } from './FeatureList'
...
export function FeatureStep({ selectedId, roles }: FeatureStepProps) {
  ...
  const activeNode = useMemo<ActiveNode | null>(() => {
    for (const mod of FEATURE_SCHEMA) {
      if (mod.id === selectedId) return { ...mod, nodeType: 'module' }
      const child = mod.children.find((res) => res.id === selectedId)
      if (child) return { ...child, nodeType: 'feature', parent: mod }
    }
    return null
  }, [selectedId])
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-900 rounded-[32px] shadow-[0_24px_64px_-16px_rgba(0,0,0,0.25)] w-full max-w-3xl h-[560px] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-200/60 dark:border-slate-800">
            ...
            <div className="flex-1 flex overflow-hidden">
              <div className="w-[280px] border-r border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/80 p-6 flex flex-col overflow-y-auto">
                <div className="mb-8">
