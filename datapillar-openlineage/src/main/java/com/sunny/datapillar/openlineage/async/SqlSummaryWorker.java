...
public class SqlSummaryWorker extends AbstractAsyncTaskWorker {
  ...
  protected void executeTask(AsyncTaskRecord task) {
    if (task.getTenantId() == null || task.getTenantId() <= 0) {
      throw new IllegalArgumentException("SQL Summary task is missing tenant information");
    }

    String sql =
        openLineageGraphDao.fetchResourceContent(
            task.getTenantId(), task.getResourceType(), task.getResourceId());
    if (sql == null || sql.isBlank()) {
      throw new IllegalStateException("SQL Summary task is missing SQL content");
    }
    ...
    double[] embedding =
        WorkerPayloadSupport.buildEmbeddingVector(summary.isBlank() ? sql : summary);
    openLineageGraphDao.writeSqlSummary(
        task.getTenantId(), task.getResourceId(), summary, tags, provider, embedding);
  }
