...
/** OpenLineage Events and asynchronous tasks DAO realize. */
@Repository
public class OpenLineageEventDaoImpl implements OpenLineageEventDao {
  ...
  public OpenLineageEventDaoImpl(
  ...
  @Override
  public void createDatasetEvent(
  ...
  public long upsertAsyncTask(TenantContext tenantContext, AsyncTaskCandidate candidate) {
    if (tenantContext == null || candidate == null || candidate.taskType() == null) {
      throw new OpenLineageWriteException("Asynchronous task storage parameters are missing");
    }
    ...
    if (id == null || id <= 0) {
      throw new OpenLineageWriteException("Asynchronous task storage failed,Task not returnedID");
    }
    return id;
  }
  ...
  @Override
  public List<AsyncTaskRecord> claimRecoverableTasks(
      AsyncTaskType taskType, int limit, String claimToken, LocalDateTime claimUntil) {
    if (taskType == null
        || limit <= 0
        || claimToken == null
        || claimToken.isBlank()
    ...
    return claimed.stream()
        .sorted(
            Comparator.comparing(
                    AsyncTaskRecord::getPriority, Comparator.nullsLast(Integer::compareTo))
  ...
  public long startAttempt(AsyncTaskAttemptRecord record) {
    if (record == null) {
      throw new OpenLineageWriteException("Task details storage parameters are missing");
    }
    mapper.insertTaskAttempt(record);
    if (record.getId() == null || record.getId() <= 0) {
      throw new OpenLineageWriteException("Failed to enter task details into the database");
    }
    return record.getId();
  }
  ...
  public void markTaskFailed(
      long taskId, String claimToken, String lastError, LocalDateTime nextRunAt, boolean dead) {
    if (taskId <= 0 || claimToken == null || claimToken.isBlank()) {
      return;
    }
  ...
  public String createBatch(AsyncBatchRecord record) {
    if (record == null) {
      throw new OpenLineageWriteException("Batch record parameter missing");
    }
  ...
  public void finishBatch(
      String batchNo, int successCount, int failedCount, String status, LocalDateTime finishedAt) {
  ...
  private void insertLineageEvent(
      OpenLineageEventEnvelope envelope, TenantContext tenantContext, String runEventType) {
  ...
  private String requireText(String value, String field) {
    if (value == null || value.isBlank()) {
      throw new OpenLineageWriteException("Field missing:%s", field);
    }
    return value;
  }
}
