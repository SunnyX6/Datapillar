package com.sunny.datapillar.openlineage.web.handler;
...
public class OpenLineageApiExceptionHandler {

  private static final Logger log = LoggerFactory.getLogger(OpenLineageApiExceptionHandler.class);
  ...
  public ErrorResponse handleIllegalArgumentException(
    ...
    String message = exception.getMessage() == null ? "Parameter error" : exception.getMessage();
    return buildErrorResponse(
        new com.sunny.datapillar.common.exception.BadRequestException(exception, message),
        response);
  }
  ...
  private ErrorResponse buildErrorResponse(Throwable throwable, HttpServletResponse response) {
    ...
    if (detail.serverError()) {
      log.error("Service exception:type={},message={}", detail.type(), detail.message(), throwable);
    } else {
      log.warn("Request exception:type={},message={}", detail.type(), detail.message(), throwable);
    }

    response.setStatus(detail.httpStatus());
    return ErrorResponse.of(detail.errorCode(), detail.type(), detail.message(), null);
  }
}
