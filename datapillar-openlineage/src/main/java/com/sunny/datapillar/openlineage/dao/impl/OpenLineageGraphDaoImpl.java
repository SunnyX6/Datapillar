...
public class OpenLineageGraphDaoImpl implements OpenLineageGraphDao {
  ...
  @Override
  public OpenLineageUpdateResult updateDatapillarModel(
...
""",
  ...
  public void writeEmbedding(
      Long tenantId, String resourceId, String provider, double[] embedding) {
    if (tenantId == null || tenantId <= 0 || resourceId == null || resourceId.isBlank()) {
      return;
    }
      ...
      session.executeWrite(
          tx -> {
            tx.run(
                """
MATCH (n {tenantId:$tenantId,id:$resourceId})
SET n.embedding = $embedding,n.embeddingProvider = $provider,n.embeddingUpdatedAt = datetime(),n.updatedAt = datetime()
""",
                Map.of(
                    "tenantId",
                    tenantId,
...
SET n.summary = $summary,n.tags = $tags,n.summaryGeneratedAt = datetime(),n.embedding = $embedding,n.embeddingProvider = $provider,n.embeddingUpdatedAt = datetime(),n.updatedAt = datetime()
""",
                Map.of(
                    "tenantId",
                    tenantId,
  ...
  private OpenLineageUpdateResult updateGraph(
      OpenLineageEventEnvelope envelope, TenantContext tenantContext, boolean includeSqlLineage) {
    ...
    try (Session session = newSession()) {
      ...
      List<AssetRef> inputRefs =
          upsertDatasets(session, envelope.inputDatasetNodes(), tenantContext);
      List<AssetRef> outputRefs =
          upsertDatasets(session, envelope.outputDatasetNodes(), tenantContext);
      OpenLineageUpdateResult.Builder resultBuilder = OpenLineageUpdateResult.builder();
      Set<String> taskDedup = new HashSet<>();
      collectEmbeddingTasks(
          inputRefs, tenantContext, resultBuilder, taskDedup, envelope.payloadFingerprint());
      collectEmbeddingTasks(
          outputRefs, tenantContext, resultBuilder, taskDedup, envelope.payloadFingerprint());
      ...
      throw new OpenLineageWriteException(ex, "write Neo4j Map failed");
    }
  }

  private List<AssetRef> upsertDatasets(
      Session session, List<JsonNode> datasets, TenantContext tenantContext) {
      ...
      session.executeWrite(
          tx -> {
            tx.run(
                ...
                SET catalog.name = $catalogName,catalog.updatedAt = datetime()
  ...
  private String upsertSqlNode(
      Session session, OpenLineageEventEnvelope envelope, TenantContext tenantContext, String sql) {
    String sqlId =
        stableId(
            tenantContext.tenantId()
                + "|sql|"
                + envelope.jobNamespace()
                + "|"
                + envelope.jobName()
                + "|"
                + sql);
    Map<String, Object> params =
        Map.of(
            ...
            sql,
            "jobNamespace",
            defaultText(envelope.jobNamespace(), "unknown"),
            "jobName",
            defaultText(envelope.jobName(), "unknown"));
    session.executeWrite(
        tx -> {
          tx.run(
              """
...
SET sql.content = $content,sql.jobNamespace = $jobNamespace,sql.jobName = $jobName,sql.updatedAt = datetime()
...
""",
  ...
  private void collectEmbeddingTasks(
      List<AssetRef> refs,
      TenantContext tenantContext,
  ...
  private AssetRef parseDataset(JsonNode dataset, Long tenantId) {
    ...
    String tableId =
        stableId(tenantId + "|table|" + catalogName + "|" + schemaName + "|" + tableName);
  ...
  private void validateTenant(TenantContext tenantContext) {
    if (tenantContext == null
      ...
      throw new OpenLineageWriteException("Missing tenantsID");
    }
    if (tenantContext.tenantCode() == null || tenantContext.tenantCode().isBlank()) {
      throw new OpenLineageWriteException("Missing tenant code");
    }
    if (tenantContext.tenantName() == null || tenantContext.tenantName().isBlank()) {
      throw new OpenLineageWriteException("Tenant name missing");
    }
  }
