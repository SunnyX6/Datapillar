...
public class EmbeddingTaskWorker extends AbstractAsyncTaskWorker {
  ...
  protected void executeTask(AsyncTaskRecord task) {
    if (task.getTenantId() == null || task.getTenantId() <= 0) {
      throw new IllegalArgumentException("Vector task is missing tenant information");
    }
    ...
    if (content == null || content.isBlank()) {
      throw new IllegalStateException("Vector tasks are missing available content");
    }
    ...
    openLineageGraphDao.writeEmbedding(
        task.getTenantId(), task.getResourceId(), provider, embedding);
  }
