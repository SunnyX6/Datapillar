...
class EmbeddingTaskWorkerTest {
  ...
  void shouldMarkTaskFailedWhenEmbeddingWriteFails() {
    ...
    when(eventDao.truncateError(any()))
        .thenReturn("STATE_ERROR: Vector tasks are missing available content");
    when(eventDao.computeNextRunAt(eq(1), any(LocalDateTime.class)))
        .thenReturn(LocalDateTime.now().plusMinutes(1));
    when(eventDao.claimRecoverableTasks(
            any(), any(Integer.class), any(String.class), any(LocalDateTime.class)))
        .thenReturn(List.of());
    ...
    verify(eventDao, timeout(3000))
        .markTaskFailed(
            eq(1L), eq("claim-1"), any(String.class), any(LocalDateTime.class), eq(false));
    verify(eventDao, timeout(3000))
        .finishAttempt(
            eq(99L),
            eq("FAILED"),
