...
class OpenLineageDaoTest {
  ...
  private final OpenLineageEventDaoImpl dao =
      new OpenLineageEventDaoImpl(mapper, new TransactionTemplate(transactionManager), 120);
  ...
  void shouldClassifyAndTruncateError() {
    Assertions.assertEquals(
        "VALIDATION_ERROR", dao.classifyErrorType(new IllegalArgumentException("bad")));
    Assertions.assertEquals(
        "STATE_ERROR", dao.classifyErrorType(new IllegalStateException("state")));
