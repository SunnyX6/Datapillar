 ...
 * 工作流广播器
 * <p>
 * 使用 Pekko Distributed Data (CRDT) 广播工作流事件给所有 Worker
 * <p>
...
public class WorkflowBroadcaster {
    ...
    private static final Logger log = LoggerFactory.getLogger(WorkflowBroadcaster.class);
    ...
    public WorkflowBroadcaster(ActorSystem<?> system) {
        this.system = system;
        ...
        log.info("WorkflowBroadcaster 初始化完成");
    }
    ...
    public void broadcast(WorkflowBroadcast event) {
        log.info("广播工作流事件: eventId={}, op={}, payload={}",
    ...
    private static class InternalActor extends AbstractBehavior<InternalCommand> {
        ...
        static Behavior<InternalCommand> create(WorkflowBroadcaster broadcaster) {
            return Behaviors.setup(context -> new InternalActor(context, broadcaster));
        }

        private InternalActor(ActorContext<InternalCommand> context, WorkflowBroadcaster broadcaster) {
            super(context);
            ...
            log.info("WorkflowBroadcaster 内部 Actor 初始化完成");
        }
