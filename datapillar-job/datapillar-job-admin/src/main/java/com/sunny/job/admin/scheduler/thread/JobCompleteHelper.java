...
public class JobCompleteHelper {
		...
		callbackThreadPool = new ThreadPoolExecutor(
				...
				new ThreadFactory() {
					...
					public Thread newThread(Runnable r) {
						return new Thread(r, "datapillar-job, admin JobLosedMonitorHelper-callbackThreadPool-" + r.hashCode());
					}
				},
				new RejectedExecutionHandler() {
					...
					public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
						r.run();
						logger.warn(">>>>>>>>>>> datapillar-job, callback too fast, match threadpool rejected handler(run now).");
					}
				});
			...
			public void run() {
				...
				while (!toStop) {
					try {
						...
						List<Long> losedJobIds  = DatapillarJobAdminConfig.getAdminConfig().getDatapillarJobLogMapper().findLostJobIds(losedTime);
						...
						if (losedJobIds!=null && losedJobIds.size()>0) {
							for (Long logId: losedJobIds) {
								...
								DatapillarJobCompleter.updateHandleInfoAndFinish(jobLog);
							}
				...
				logger.info(">>>>>>>>>>> datapillar-job, JobLosedMonitorHelper stop");

			}
		});
		monitorThread.setDaemon(true);
		monitorThread.setName("datapillar-job, admin JobLosedMonitorHelper");
		monitorThread.start();
	}
	...
	private ReturnT<String> callback(HandleCallbackParam handleCallbackParam) {
		// valid log item
		DatapillarJobLog log = DatapillarJobAdminConfig.getAdminConfig().getDatapillarJobLogMapper().load(handleCallbackParam.getLogId());
		if (log == null) {
		...
		// 完成 Future（在写数据库之前，log 已有完整执行结果）
		JobCallbackFutureHolder.completeFuture(log.getJobId(), log);
