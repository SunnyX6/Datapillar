 ...
 * Bucket 状态管理器
 * <p>
 * 使用 Pekko Distributed Data (CRDT) 在集群中同步 Bucket 所有权
 * 同时通过 DB 持久化租约信息，用于 Worker 重启时恢复
 * <p>
 * 数据结构: LWWMap[Integer, BucketLease]
...
public class BucketStateManager {
    ...
    private static final String BUCKET_STATE_KEY = "bucket-states";
    ...
    public BucketStateManager(ActorSystem<?> system, WorkerStateManager workerStateManager) {
        this(system, BucketLease.DEFAULT_BUCKET_COUNT, workerStateManager, null);
    }

    public BucketStateManager(ActorSystem<?> system, int bucketCount, WorkerStateManager workerStateManager) {
        this(system, bucketCount, workerStateManager, null);
    }

    public BucketStateManager(ActorSystem<?> system, int bucketCount,
        ...
        log.info("BucketStateManager 初始化完成，selfAddress={}, bucketCount={}, dbPersistence={}",
     ...
     * CRDT 数据通过 Update 操作在集群中同步，本地监听通过回调实现
     */
    private void subscribeToCrdt() {
        // typed API 的 Subscribe 需要 ActorRef，在非 Actor 组件中使用本地缓存模式
        // CRDT 数据通过 Update 操作在集群中同步
        log.info("Bucket CRDT 订阅已配置（本地缓存模式）");
    }
    ...
    public void renewBucket(int bucketId) {
        ...
        if (lease != null && lease.isOwnedBy(selfAddress)) {
            ...
            updateBucketLease(renewedLease);
        }
    }
    ...
    public void releaseBucket(int bucketId) {
        ...
        Key<LWWMap<Integer, BucketLease>> key = createKey();
        replicator.tell(new Replicator.Update<>(
                key,
                LWWMap.empty(),
                new Replicator.WriteMajority(WRITE_TIMEOUT),
                system.ignoreRef(),
                existing -> existing.remove(selfUniqueAddress, bucketId)
        ));
     ...
     * 更新 Bucket 租约到 CRDT
     */
    private void updateBucketLease(BucketLease lease) {
        localCache.put(lease.bucketId(), lease);
        ...
        replicator.tell(new Replicator.Update<>(
                ...
                new Replicator.WriteMajority(WRITE_TIMEOUT),
                system.ignoreRef(),
                existing -> existing.put(selfUniqueAddress, lease.bucketId(), lease)
        ));
    }
     ...
     * 创建 CRDT Key
     */
    private Key<LWWMap<Integer, BucketLease>> createKey() {
        return LWWMapKey.create(BUCKET_STATE_KEY);
    }
