package com.sunny.datapillar.admin.module.workflow.service.impl;
...
import com.sunny.datapillar.admin.module.workflow.dag.DagBuilder;
import com.sunny.datapillar.admin.module.workflow.dag.DagValidationException;
...
import com.sunny.datapillar.admin.module.workflow.entity.JobDependency;
import com.sunny.datapillar.admin.module.workflow.entity.JobInfo;
import com.sunny.datapillar.admin.module.workflow.entity.JobWorkflow;
import com.sunny.datapillar.admin.module.workflow.mapper.JobDependencyMapper;
...
public class JobDependencyServiceImpl implements JobDependencyService {
    ...
    public Long createDependency(Long workflowId, JobDependencyDto.Create dto) {
        ...
        if (job == null || !job.getWorkflowId().equals(workflowId)) {
            throw new WebAdminException(WebAdminErrorCode.JOB_NOT_FOUND, dto.getJobId());
        }

        JobInfo parentJob = jobInfoMapper.selectById(dto.getParentJobId());
        if (parentJob == null || !parentJob.getWorkflowId().equals(workflowId)) {
            throw new WebAdminException(WebAdminErrorCode.JOB_NOT_FOUND, dto.getParentJobId());
        }

        // 检查依赖是否已存在
        if (dependencyMapper.existsDependency(dto.getJobId(), dto.getParentJobId()) > 0) {
            throw new WebAdminException(WebAdminErrorCode.DEPENDENCY_EXISTS);
        }
    ...
    public void deleteDependency(Long jobId, Long parentJobId) {
        ...
        if (deleted == 0) {
            throw new WebAdminException(WebAdminErrorCode.DEPENDENCY_NOT_FOUND);
        }
