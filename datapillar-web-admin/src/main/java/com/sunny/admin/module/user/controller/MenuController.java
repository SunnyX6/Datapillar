...
public class MenuController {
    ...
    @GetMapping("/me")
    public WebAdminResponse<List<MenuRespDto>> getCurrentUserMenus() {
    ...
    public WebAdminResponse<List<MenuRespDto>> getVisibleMenus() {
    ...
    @GetMapping("/tree")
    @PreAuthorize("hasAuthority('ADMIN')")
    public WebAdminResponse<List<MenuRespDto>> getMenuTree() {
        List<MenuRespDto> allMenus = menuService.getAllVisibleMenus();
        List<MenuRespDto> menuTree = menuService.buildMenuTree(allMenus);
        return WebAdminResponse.ok(menuTree);
    }

    @Operation(summary = "获取所有权限列表", description = "获取系统中所有菜单权限的列表（仅管理员）")
    @GetMapping("/permissions")
    @PreAuthorize("hasAuthority('ADMIN')")
    public WebAdminResponse<List<PermissionRespDto>> getAllPermissions() {
        List<Permission> permissions = permissionMapper.selectList(null);
        List<PermissionRespDto> responses = permissions.stream()
                .map(this::convertToPermissionResponse)
                .collect(Collectors.toList());
        return WebAdminResponse.ok(responses);
    }

    @Operation(summary = "根据ID获取权限详情", description = "根据权限ID获取权限的详细信息（仅管理员）")
    @GetMapping("/permissions/{id}")
    @PreAuthorize("hasAuthority('ADMIN')")
    public WebAdminResponse<PermissionRespDto> getPermissionById(@PathVariable Long id) {
        Permission permission = permissionMapper.selectById(id);
        if (permission == null) {
            return WebAdminResponse.error("PERMISSION_NOT_FOUND", "权限不存在");
        }
        return WebAdminResponse.ok(convertToPermissionResponse(permission));
    }
