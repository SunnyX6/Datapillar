...
class StudioDbExceptionTranslatorTest {
  ...
  void map_shouldResolveInvitationEmailDuplicateByConstraint() {
    DbUniqueConstraintViolationException dbException =
        new DbUniqueConstraintViolationException(
            new RuntimeException("duplicate"), 1062, "23000", "uq_user_email");

    RuntimeException runtimeException =
        translator.map(StudioDbScene.STUDIO_INVITATION_REGISTER, dbException);
  ...
  void map_shouldResolveLlmProviderDuplicateByConstraint() {
    DbUniqueConstraintViolationException dbException =
        new DbUniqueConstraintViolationException(
            new RuntimeException("duplicate"), 1062, "23000", "uq_ai_provider_code");

    RuntimeException runtimeException =
        translator.map(StudioDbScene.STUDIO_LLM_PROVIDER_CREATE, dbException);
  ...
  void map_shouldFallbackToGenericAlreadyExistsWhenConstraintUnknown() {
    DbUniqueConstraintViolationException dbException =
        new DbUniqueConstraintViolationException(
            new RuntimeException("duplicate"), 1062, "23000", "uq_unknown");
    ...
    Assertions.assertEquals("Data already exists", runtimeException.getMessage());
  }

  @Test
  void map_shouldMapDeadlockToServiceUnavailable() {
    DbDeadlockException dbException =
        new DbDeadlockException(new RuntimeException("deadlock"), 1213, "40001", null);

    RuntimeException runtimeException = translator.map(StudioDbScene.STUDIO_GENERIC, dbException);

    Assertions.assertEquals("Service unavailable", runtimeException.getMessage());
  }
}
