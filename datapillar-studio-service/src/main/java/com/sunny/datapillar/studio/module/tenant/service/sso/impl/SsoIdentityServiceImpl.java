...
/**
 * Single sign-onIdentityService implementation Implement single sign-onIdentityBusiness process and
 * rule verification
 *
 * @author Sunny
 * @date 2026-01-01
...
public class SsoIdentityServiceImpl implements SsoIdentityService {
  ...
  public List<SsoIdentityItem> list(String provider, Long userId) {
    ...
    queryWrapper.orderByDesc(UserIdentity::getUpdatedAt).orderByDesc(UserIdentity::getId);
  ...
  public Long bindByCode(SsoIdentityBindByCodeRequest request) {
    ...
    TenantUser tenantUser =
        tenantUserMapper.selectByTenantIdAndUserId(tenantId, request.getUserId());
    ...
    DingtalkUserInfo dingtalkUserInfo =
        dingtalkBindingClient.fetchUserInfo(clientId, clientSecret, request.getAuthCode());
  ...
  private LambdaQueryWrapper<UserIdentity> buildListWrapper(
      Long tenantId, String provider, Long userId) {
  ...
  private TenantSsoConfig loadEnabledConfig(Long tenantId, String provider) {
    ...
    wrapper
        .eq(TenantSsoConfig::getTenantId, tenantId)
