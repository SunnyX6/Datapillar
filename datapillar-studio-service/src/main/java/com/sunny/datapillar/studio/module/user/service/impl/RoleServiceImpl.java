...
public class RoleServiceImpl implements RoleService {
  ...
  public Long createRole(RoleCreateRequest dto) {
    ...
    if (ROLE_TYPE_USER.equals(roleType) && role.getId() != null) {
      assignDefaultPermsForUserRole(tenantId, role.getId());
    }
    if (role.getId() == null) {
  ...
  public void removeRoleMembers(Long roleId, List<Long> userIds) {
    ...
    List<Long> protectedUserIds =
        userMapper.selectUserIdsByMaxLevel(
            tenantId, uniqueUserIds, ROLE_LEVEL_PLATFORM_SUPER_ADMIN);
    if (protectedUserIds != null && !protectedUserIds.isEmpty()) {
      throw new com.sunny.datapillar.common.exception.ForbiddenException(
          "Platform super users are not allowed to remove roles");
    }
  ...
  public void updateRolePermissions(Long roleId, List<RoleFeatureAssignmentItem> permissions) {
    ...
    for (Map.Entry<Long, RoleFeatureAssignmentItem> entry : uniqueAssignments.entrySet()) {
      ...
      Permission resolvedPermission =
          resolvePermission(assignment, permissionMap, permissionByIdMap);
      Long permissionId = resolvedPermission.getId();
      TenantFeaturePermissionLimitItem limit = limitMap.get(entry.getKey());
      if (limit == null || limit.getStatus() == null || limit.getStatus() != STATUS_ENABLED) {
        throw new com.sunny.datapillar.common.exception.ForbiddenException("No access");
      }
      int limitLevel = limit.getPermissionLevel() == null ? 0 : limit.getPermissionLevel();
      int permissionLevel =
          resolvedPermission.getLevel() == null ? 0 : resolvedPermission.getLevel();
  ...
  private void assignDefaultPermsForUserRole(Long tenantId, Long roleId) {
    ...
    for (FeatureObject object : featureObjects) {
      if (object == null
          || object.getId() == null
          || !shouldAssignDefaultPermission(object, manageDefineCategoryId)) {
        continue;
      }
      TenantFeaturePermissionLimitItem permissionLimit = limitMap.get(object.getId());
      if (permissionLimit == null
          || permissionLimit.getStatus() == null
          || permissionLimit.getStatus() != STATUS_ENABLED) {
        continue;
      }
      int limitLevel =
          permissionLimit.getPermissionLevel() == null ? 0 : permissionLimit.getPermissionLevel();
  ...
  private Long resolveManageDefineCategoryId() {
    FeatureObjectCategory category =
        featureObjectCategoryMapper.selectByCode(CATEGORY_CODE_MANAGE_DEFINE);
  ...
  private boolean shouldAssignDefaultPermission(FeatureObject object, Long manageDefineCategoryId) {
    ...
    if (PROFILE_ROOT_PATH.equals(path)
        || (path != null && path.startsWith(PROFILE_ROOT_PATH + "/"))) {
      return true;
    }
  ...
  private void ensureRoleMutable(Role role) {
    if (role != null
        && role.getLevel() != null
        && role.getLevel() <= ROLE_LEVEL_PLATFORM_SUPER_ADMIN) {
      throw new com.sunny.datapillar.common.exception.ForbiddenException(
          "Platform super management roles are not allowed to be modified or deleted.");
    }
  }

  private void applyTenantPermissionLimit(
      List<FeatureObjectPermissionItem> items, Map<String, Permission> permissionMap) {
  ...
  private List<FeatureObjectPermissionItem> buildPermissionTree(
      List<FeatureObjectPermissionItem> items) {
    if (items == null || items.isEmpty()) {
  ...
  private void sortPermissionNodes(List<FeatureObjectPermissionItem> nodes) {
    ...
    nodes.sort(
        Comparator.comparingInt(
                (FeatureObjectPermissionItem item) -> item.getSort() == null ? 0 : item.getSort())
  ...
  private Permission resolvePermission(
      RoleFeatureAssignmentItem assignment,
    ...
    if (assignment.getPermissionId() != null) {
      ...
      if (permission == null) {
        throw new com.sunny.datapillar.common.exception.BadRequestException(
            "Parameter error", String.valueOf(assignment.getPermissionId()));
      }
      return permission;
    }
    ...
    if (permission == null) {
      throw new com.sunny.datapillar.common.exception.BadRequestException(
          "Parameter error", assignment.getPermissionCode());
    }
    return permission;
  }
}
