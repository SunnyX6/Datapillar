...
/**
 * Studio DB Exception business converter
 *
 * @author Sunny
 * @date 2026-02-26
 */
...
public class StudioDbExceptionTranslator {
  ...
  public DatapillarRuntimeException map(StudioDbScene scene, DbStorageException dbException) {
    if (dbException
        instanceof DbUniqueConstraintViolationException uniqueConstraintViolationException) {
    ...
    if (dbException instanceof DbForeignKeyViolationException
      ...
      return new BadRequestException(dbException, "Parameter error");
    }
    ...
    if (dbException instanceof DbDeadlockException
      ...
      return new ServiceUnavailableException(dbException, "Service unavailable");
    }

    return new InternalException(
        dbException, ErrorType.STUDIO_DB_INTERNAL, Map.of(), "Server internal error");
  }

  private DatapillarRuntimeException mapUnique(
      StudioDbScene scene, DbUniqueConstraintViolationException dbException) {
    ...
    if (scene == StudioDbScene.STUDIO_SSO_CONFIG && "uq_tenant_sso".equals(constraintName)) {
      return new SsoConfigAlreadyExistsException(dbException);
    }

    if (scene == StudioDbScene.STUDIO_TENANT_MANAGE && "uq_tenant_code".equals(constraintName)) {
    ...
    return new AlreadyExistsException(
        dbException, ErrorType.STUDIO_DB_DUPLICATE, Map.of(), "Data already exists");
  }
}
