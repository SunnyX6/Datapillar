...
/** User service implementation Implement user business process and rule verification. */
@Slf4j
@Service
...
public class UserServiceImpl implements UserService {
  ...
  public Long createUser(UserCreateRequest dto) {
    ...
    if (existing != null) {
      ...
      if (tenantUser != null
          && tenantUser.getStatus() != null
          && tenantUser.getStatus() == MEMBER_STATUS_ENABLED) {
        throw new com.sunny.datapillar.common.exception.AlreadyExistsException(
            "Username already exists: %s", dto.getUsername());
      }
      if (tenantUser == null) {
    ...
    if (dto.getUsername() != null) {
      ...
      if (userWithSameName != null && !userWithSameName.getId().equals(id)) {
        throw new com.sunny.datapillar.common.exception.ConflictException(
            "Username is already taken by another user: %s", dto.getUsername());
      }
  ...
  private List<Long> normalizeRoleIds(List<Long> roleIds) {
    LinkedHashSet<Long> uniqueRoles = new LinkedHashSet<>();
    if (roleIds != null) {
  ...
  public List<FeatureObjectPermissionItem> getUserPermissions(Long userId) {
    ...
    List<FeatureObjectPermissionItem> objects =
        featureObjectMapper.selectFeatureObjectsAll(tenantId);
    ...
    Map<Long, List<FeatureRoleSourceItem>> roleSourceMap =
        roleSources == null
            ? new HashMap<>()
            : roleSources.stream()
                .collect(Collectors.groupingBy(FeatureRoleSourceItem::getObjectId));
    ...
    for (FeatureObjectPermissionItem object : objects) {
      ...
      object.setPermissionCode(
          PermissionLevelUtil.minCode(permissionMap, rolePermission, tenantLimit));
      result.add(object);
    }
    return result;
  }
  ...
  private String getRequiredTenantCode(Long tenantId) {
