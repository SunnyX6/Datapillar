...
/**
 * Invitation service implementation Implement invitation business process and rule verification
 *
 * @author Sunny
 * @date 2026-01-01
 */
...
public class InvitationServiceImpl implements InvitationService {
  ...
  private static final String ROLE_CODE_TEMPLATE = "dr_t%d_r%d";
  private static final String STEP_GRAVITINO_PROVISION = "GRAVITINO_PROVISION";
  private static final String STEP_LOCAL_FINALIZE = "LOCAL_FINALIZE";
  ...
  public InvitationCreateResponse createInvitation(InvitationCreateRequest dto) {
    ...
    if (tenant == null) {
      throw new InvitationTenantNotFoundException("Tenant does not exist: %s", tenantId);
    }
    ...
    if (activeInvitation != null) {
      ...
      OffsetDateTime responseExpiresAt =
  ...
  public void registerInvitation(InvitationRegisterRequest request) {
    ...
    InvitationProvisionContext context =
        prepareInvitationProvisioning(inviteCode, username, email, password);
    try {
      completeInvitationProvisioning(
          context.invitationId(), context.userId(), context.operationId());
    } catch (RuntimeException rpcException) {
      failInvitationProvisioning(
          context.invitationId(),
          context.tenantId(),
          context.userId(),
          context.operationId(),
          rpcException);
      throw new InvitationInternalException(rpcException);
    }
  }
  ...
  private InvitationProvisionContext prepareInvitationProvisioning(
      String inviteCode, String username, String email, String password) {
          ...
          return withTenantContext(
              ...
              () -> {
                ...
                RegisterPrincipal principal =
                    resolveRegisterPrincipal(invitation, tenantId, username, email, password, now);
                ensureTenantMember(tenantId, principal.userId(), now);
                grantInvitationRoles(invitation.getId(), tenantId, principal.userId(), now);
                String operationId =
                    markInvitationProvisioning(
                        invitation.getId(),
                        invitation.getInviteCode(),
                        tenantId,
                        principal.userId(),
                        now);
                return new InvitationProvisionContext(
                    invitation.getId(),
                    tenantId,
                    tenant.getCode().trim(),
                    principal.userId(),
  ...
  private RegisterPrincipal resolveRegisterPrincipal(
    ...
    if (status == STATUS_FAILED || status == STATUS_PROVISIONING) {
      Long userId = invitation.getAcceptedUserId();
      ...
      String resolvedUsername =
          StringUtils.hasText(existingUser.getUsername())
              ? existingUser.getUsername().trim()
              : username;
      return new RegisterPrincipal(userId, resolvedUsername);
    }
  ...
  private void completeInvitationProvisioning(Long invitationId, Long userId, String operationId) {
    runInTransactionWithoutResult(
        () -> markInvitationAccepted(invitationId, userId, operationId, LocalDateTime.now()));
  }

  private void failInvitationProvisioning(
      Long invitationId,
      Long tenantId,
      Long userId,
      String operationId,
      RuntimeException rpcException) {
    try {
      runInTransactionWithoutResult(
          () ->
              markInvitationFailed(
                  invitationId, tenantId, userId, operationId, LocalDateTime.now(), rpcException));
    } catch (RuntimeException ignored) {
      // Keep the original RPC failure as the primary exception.
    }
  }
  ...
  private Long getInvitationRoleId(Long invitationId) {
    ...
    roleQuery
        .eq(UserInvitationRole::getInvitationId, invitationId)
  ...
  private void validatePendingInvitationStatus(UserInvitation invitation, LocalDateTime now) {
    Integer status = invitation.getStatus();
  ...
  private void markInvitationExpired(Long invitationId, LocalDateTime now) {
    ...
    update
        .eq(UserInvitation::getId, invitationId)
  ...
  private String markInvitationProvisioning(
      Long invitationId, String inviteCode, Long tenantId, Long userId, LocalDateTime now) {
    LambdaUpdateWrapper<UserInvitation> update = new LambdaUpdateWrapper<>();
    update
        .eq(UserInvitation::getId, invitationId)
        .in(UserInvitation::getStatus, STATUS_PENDING, STATUS_FAILED, STATUS_PROVISIONING)
    ...
    return "invite:" + invitationId + ":" + userId;
  }

  private void markInvitationAccepted(
      Long invitationId, Long userId, String operationId, LocalDateTime now) {
    LambdaUpdateWrapper<UserInvitation> update = new LambdaUpdateWrapper<>();
    update
        .eq(UserInvitation::getId, invitationId)
    ...
    int updated = userInvitationMapper.update(null, update);
    if (updated == 0) {
      UserInvitation latest = userInvitationMapper.selectById(invitationId);
      if (latest != null && latest.getStatus() != null && latest.getStatus() == STATUS_ACCEPTED) {
        return;
      }
      throw new InvitationAlreadyUsedException();
    }
  ...
  private void markInvitationFailed(
      Long invitationId,
      Long tenantId,
      Long userId,
      String operationId,
  ...
  private void grantInvitationRoles(
      Long invitationId, Long tenantId, Long userId, LocalDateTime now) {
    LambdaQueryWrapper<UserInvitationRole> roleQuery = new LambdaQueryWrapper<>();
  ...
  private UserInvitation findActiveInvitationForRole(
      Long tenantId, Long roleId, LocalDateTime now) {
    LambdaQueryWrapper<UserInvitationRole> roleQuery = new LambdaQueryWrapper<>();
  ...
  private InvitationCreateResponse buildCreateResponse(
      UserInvitation invitation, OffsetDateTime expiresAt, Tenant tenant, Role role, User inviter) {
  ...
  private String buildInviteUri(String inviteCode) {
    UriComponentsBuilder builder =
        UriComponentsBuilder.fromPath("/invite").queryParam("inviteCode", inviteCode);
    return builder.encode().build().toUriString();
  }

  private record InvitationProvisionContext(
      Long invitationId,
      Long tenantId,
      String tenantCode,
      Long userId,
  ...
  private record RegisterPrincipal(Long userId, String username) {}
}
