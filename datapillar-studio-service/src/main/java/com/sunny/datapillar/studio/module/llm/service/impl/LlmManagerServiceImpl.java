...
/**
 * large modelManagerService implementation Implement large modelsManagerBusiness process and rule
 * verification
 *
 * @author Sunny
 * @date 2026-01-01
...
public class LlmManagerServiceImpl implements LlmManagerService {
  ...
  private static final Set<String> OPENAI_COMPATIBLE_PROVIDERS =
      Set.of("openai", "deepseek", "openrouter", "ollama", "glm", "mistral", "google", "meta");
  ...
  public void createProvider(Long userId, LlmProviderCreateRequest request) {
    ...
    if (request == null) {
      throw new LlmBadRequestException("Parameter error");
    }
    ...
    try {
      ...
      if (inserted == 0 || provider.getId() == null) {
        throw new LlmInternalException("Server internal error");
      }
  ...
  public void updateProvider(Long userId, String providerCode, LlmProviderUpdateRequest request) {
    ...
    if (provider == null) {
      throw new LlmNotFoundException("Supplier does not exist");
    }

    boolean updated = false;
    if (request.getName() != null) {
      provider.setName(normalizeRequiredText(request.getName(), "name cannot be empty"));
      updated = true;
    }
    ...
    if (!addModelIds.isEmpty() || !removeModelIds.isEmpty()) {
      ...
      if (!intersection.isEmpty()) {
        throw new LlmBadRequestException(
            "add_model_ids with remove_model_ids No duplicate values allowed");
      }
    ...
    if (!updated) {
      throw new LlmBadRequestException("No fields to update");
    }
  ...
  public void deleteProvider(Long userId, String providerCode) {
    ...
    if (modelCount != null && modelCount > 0) {
      throw new LlmBadRequestException("Model exists under supplier，cannot be deleted");
    }
  ...
  public List<LlmModelResponse> listModels(
    ...
    if (StringUtils.hasText(keyword)) {
      ...
      wrapper.and(
          w ->
              w.like(AiModel::getProviderModelId, normalizedKeyword)
                  .or()
  ...
  public LlmModelResponse createModel(Long userId, LlmModelCreateRequest request) {
    ...
    String providerModelId =
        normalizeRequiredText(request.getProviderModelId(), "provider_model_id cannot be empty");

    if (request.getModelType() == AiModelType.EMBEDDINGS
        && request.getEmbeddingDimension() == null) {
      throw new LlmBadRequestException("embedding_dimension cannot be empty");
    }

    AiModel model = new AiModel();
    model.setTenantId(tenantId);
    model.setProviderModelId(providerModelId);
    model.setName(normalizeRequiredText(request.getName(), "name cannot be empty"));
    ...
    if (StringUtils.hasText(normalizedApiKey)) {
      ...
      if (!StringUtils.hasText(resolvedBaseUrl)) {
        throw new LlmBadRequestException("base_url cannot be empty");
      }
    ...
    grantModelToPlatformAdmin(tenantId, operatorUserId, model.getId());
    return toModelResponse(model, provider, tenantCode);
  }
  ...
  public void deleteModel(Long userId, Long aiModelId) {
    ...
    grantWrapper
        .eq(AiModelGrant::getTenantId, tenantId)
    ...
    if (grantCount != null && grantCount > 0) {
      throw new LlmBadRequestException("Model is authorized，cannot be deleted");
    }

    LambdaQueryWrapper<AiUsage> usageWrapper = new LambdaQueryWrapper<>();
    usageWrapper.eq(AiUsage::getTenantId, tenantId).eq(AiUsage::getModelId, model.getId());
    Long usageCount = aiUsageMapper.selectCount(usageWrapper);
    if (usageCount != null && usageCount > 0) {
      throw new LlmBadRequestException("The model has usage records，cannot be deleted");
    }
  ...
  public LlmModelConnectResponse connectModel(
    ...
    AiProvider provider = aiProviderMapper.selectById(model.getProviderId());
    if (provider == null) {
      throw new LlmBadRequestException("Supplier does not exist");
    }

    String apiKey = normalizeRequiredText(request.getApiKey(), "api_key cannot be empty");
    String resolvedBaseUrl = normalizeNullableText(request.getBaseUrl());
    if (!StringUtils.hasText(resolvedBaseUrl)) {
  ...
  @Override
  public List<LlmUserModelUsageResponse> listUserModelUsages(
  ...
  public List<LlmUserModelPermissionResponse> listUserModelPermissions(
    ...
    Long tenantId = getRequiredTenantId();
    requireTenantUserExists(tenantId, targetUserId);
    return listModelPermissionsByUser(
        tenantId, targetUserId, onlyEnabled, getPermissionMapByCode());
  }
  ...
  public void upsertUserModelGrant(
    ...
    if (expiresAt != null && !expiresAt.isAfter(LocalDateTime.now())) {
      throw new LlmBadRequestException("expires_at Must be later than current time");
    }
    if (!isDisablePermission && model.getStatus() != AiModelStatus.ACTIVE) {
      throw new LlmBadRequestException("Only connected models can be authorized");
    }
  ...
  @Override
  @Transactional
  public LlmUserModelUsageResponse setUserDefaultModel(
      Long operatorUserId, Long targetUserId, Long aiModelId) {
    Long tenantId = getRequiredTenantId();
    Long operatorId = getRequiredUserId(operatorUserId);
    requireTenantUserExists(tenantId, targetUserId);
    Map<String, Permission> permissionByCode = getPermissionMapByCode();
    ...
    if (model.getStatus() != AiModelStatus.ACTIVE) {
      throw new LlmBadRequestException("Only connected models can be set as default");
    }
    ...
    if (!isUsablePermission(permissionByCode, permission) || isGrantExpired(grant)) {
      throw new LlmForbiddenException("No access");
    }
    ...
    return toModelUsageResponse(
        refreshed, permissionById.get(refreshed.getPermissionId()), usage, model, provider);
  }

  private List<LlmUserModelUsageResponse> listModelUsagesByUser(
      Long tenantId, Long userId, boolean onlyEnabled, Map<String, Permission> permissionByCode) {
    Map<Long, Permission> permissionById = toPermissionMapById(permissionByCode.values());
    LambdaQueryWrapper<AiModelGrant> wrapper = new LambdaQueryWrapper<>();
    wrapper.eq(AiModelGrant::getTenantId, tenantId).eq(AiModelGrant::getUserId, userId);
    wrapper.orderByDesc(AiModelGrant::getUpdatedAt).orderByDesc(AiModelGrant::getId);
    ...
    for (AiModelGrant grant : grants) {
      Permission permission = permissionById.get(grant.getPermissionId());
      if (onlyEnabled
          && (!isUsablePermission(permissionByCode, permission) || isGrantExpired(grant))) {
        continue;
      }
  ...
  private List<LlmUserModelPermissionResponse> listModelPermissionsByUser(
      Long tenantId, Long userId, boolean onlyEnabled, Map<String, Permission> permissionByCode) {
    Map<Long, Permission> permissionById = toPermissionMapById(permissionByCode.values());
    LambdaQueryWrapper<AiModelGrant> wrapper = new LambdaQueryWrapper<>();
    wrapper.eq(AiModelGrant::getTenantId, tenantId).eq(AiModelGrant::getUserId, userId);
  ...
  private AiModel getModelOrThrow(Long aiModelId, Long tenantId) {
    ...
    LambdaQueryWrapper<AiModel> wrapper = new LambdaQueryWrapper<>();
    wrapper.eq(AiModel::getTenantId, tenantId).eq(AiModel::getId, aiModelId);
    AiModel model = aiModelMapper.selectOne(wrapper);
    if (model == null) {
      throw new LlmNotFoundException("Resource does not exist");
    }
    return model;
  }
  ...
  private AiUsage getUsageNullable(Long tenantId, Long userId, Long aiModelId) {
    ...
    wrapper
        .eq(AiUsage::getTenantId, tenantId)
  ...
  private Map<Long, AiModel> loadModelsByIds(Long tenantId, Set<Long> modelIds) {
    ...
    LambdaQueryWrapper<AiModel> wrapper = new LambdaQueryWrapper<>();
    wrapper.eq(AiModel::getTenantId, tenantId).in(AiModel::getId, modelIds);
    List<AiModel> models = aiModelMapper.selectList(wrapper);
  ...
  private Map<String, Permission> getPermissionMapByCode() {
    ...
    if (permissions == null || permissions.isEmpty()) {
      throw new LlmInternalException("Permissions dictionary not configured");
    }
    ...
    if (!map.containsKey(PERMISSION_DISABLE)
        || !map.containsKey(PERMISSION_READ)
        || !map.containsKey(PERMISSION_ADMIN)) {
      throw new LlmInternalException(
          "Permission dictionary is missing, model authorization basic permissions");
    }
    return map;
  }
  ...
  private Permission resolvePermissionByCode(
      Map<String, Permission> permissionByCode, String permissionCode) {
    String normalized = normalizePermissionCode(permissionCode);
    if (normalized == null) {
      throw new LlmBadRequestException("permission_code cannot be empty");
    }
    Permission permission = permissionByCode.get(normalized);
    if (permission == null) {
      throw new LlmBadRequestException("permission_code Invalid");
    }
    return permission;
  }

  private boolean isDisablePermission(
      Map<String, Permission> permissionByCode, String permissionCode) {
    ...
    int disableLevel =
        disablePermission == null || disablePermission.getLevel() == null
            ? 0
            : disablePermission.getLevel();
    int currentLevel =
        currentPermission == null || currentPermission.getLevel() == null
            ? 0
            : currentPermission.getLevel();
    return currentLevel <= disableLevel;
  }

  private boolean isUsablePermission(
      Map<String, Permission> permissionByCode, Permission permission) {
    if (permission == null) {
      return false;
    }
    Permission disablePermission = permissionByCode.get(PERMISSION_DISABLE);
  ...
  private void requireTenantUserExists(Long tenantId, Long userId) {
    ...
    if (user == null) {
      throw new LlmNotFoundException("User does not exist: %s", userId);
    }
  }

  private void grantModelToPlatformAdmin(Long tenantId, Long operatorUserId, Long aiModelId) {
  ...
  private Long getRequiredUserId(Long userId) {
    if (userId == null || userId <= 0) {
      throw new LlmUnauthorizedException("Unauthorized access");
    }
    return userId;
  }
  ...
  private LlmUserModelUsageResponse toModelUsageResponse(
      AiModelGrant grant,
    ...
    response.setPermissionCode(
        permission == null ? null : normalizePermissionCode(permission.getCode()));
    ...
    response.setCallCount(
        decimalToString(usage == null ? null : toBigDecimal(usage.getCallCount())));
    response.setPromptTokens(
        decimalToString(usage == null ? null : toBigDecimal(usage.getPromptTokens())));
    response.setCompletionTokens(
        decimalToString(usage == null ? null : toBigDecimal(usage.getCompletionTokens())));
    response.setTotalTokens(
        decimalToString(usage == null ? null : toBigDecimal(usage.getTotalTokens())));
  ...
  private LlmUserModelPermissionResponse toModelPermissionResponse(
      AiModelGrant grant, Permission permission, AiModel model, AiProvider provider) {
  ...
  private List<String> parseJsonList(String value) {
    ...
    try {
      ...
      throw new LlmInternalException(ex, "Server internal error");
    }
  }
  ...
  private String writeJsonList(List<String> values) {
    ...
    try {
      ...
      throw new LlmBadRequestException("JSON Serialization failed", ex);
    }
  }

  private String resolveProviderName(String name, String providerCode) {
    if (StringUtils.hasText(name)) {
      return normalizeRequiredText(name, "name cannot be empty");
    }
  ...
  private List<String> normalizeIncomingModelIds(List<String> modelIds) {
    ...
    LinkedHashSet<String> normalizedIds = new LinkedHashSet<>();
    for (String modelId : modelIds) {
      normalizedIds.add(normalizeRequiredText(modelId, "model_id cannot be empty"));
    }
    return new ArrayList<>(normalizedIds);
  }
  ...
  private String normalizeProviderCode(String providerCode) {
    String normalized = normalizeRequiredText(providerCode, "provider_code cannot be empty");
    if (normalized.length() > 32) {
      throw new LlmBadRequestException("provider_code The length cannot exceed 32");
    }
    return normalized.toLowerCase(Locale.ROOT);
  }

  void verifyModelConnection(
      String providerCode,
      String providerModelId,
    ...
    String normalizedProviderModelId =
        normalizeRequiredText(providerModelId, "provider_model_id cannot be empty");
    String normalizedApiKey = normalizeRequiredText(apiKey, "api_key cannot be empty");
    ...
    if (!OPENAI_COMPATIBLE_PROVIDERS.contains(normalizedProvider)) {
      throw new LlmBadRequestException("Unsupported vendor: " + normalizedProvider);
    }

    if (modelType == AiModelType.EMBEDDINGS) {
      verifyOpenAiEmbeddings(
          normalizedProvider, normalizedProviderModelId, normalizedApiKey, normalizedBaseUrl);
      return;
    }
  ...
  private void verifyOpenAiChat(
      String providerCode, String providerModelId, String apiKey, String baseUrl) {
    OpenAiApi openAiApi = buildOpenAiApi(providerCode, apiKey, baseUrl);
    OpenAiChatOptions options =
        OpenAiChatOptions.builder().model(providerModelId).temperature(0D).maxTokens(1).build();
    ...
    if (response == null
      ...
      throw new LlmConnectionFailedException("Connection failed，please check API Key/Base URL");
    }
  }

  private void verifyOpenAiEmbeddings(
      String providerCode, String providerModelId, String apiKey, String baseUrl) {
    OpenAiApi openAiApi = buildOpenAiApi(providerCode, apiKey, baseUrl);
    OpenAiEmbeddingOptions options =
        OpenAiEmbeddingOptions.builder().model(providerModelId).build();
    OpenAiEmbeddingModel embeddingModel =
        new OpenAiEmbeddingModel(
            openAiApi, MetadataMode.EMBED, options, RetryUtils.DEFAULT_RETRY_TEMPLATE);
    EmbeddingResponse response =
        embeddingModel.call(new EmbeddingRequest(List.of("ping"), options));
  ...
  private void verifyAnthropicChat(String providerModelId, String apiKey, String baseUrl) {
    AnthropicApi anthropicApi = AnthropicApi.builder().baseUrl(baseUrl).apiKey(apiKey).build();
    AnthropicChatOptions options =
        AnthropicChatOptions.builder().model(providerModelId).temperature(0D).maxTokens(1).build();
  ...
  private String normalizeBaseUrl(String baseUrl) {
    String normalized = normalizeRequiredText(baseUrl, "base_url cannot be empty");
