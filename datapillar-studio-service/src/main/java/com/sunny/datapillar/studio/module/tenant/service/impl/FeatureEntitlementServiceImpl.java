...
/**
 * FunctionEntitlementService implementation Implement functionEntitlementBusiness process and rule
 * verification
 *
 * @author Sunny
 * @date 2026-01-01
...
public class FeatureEntitlementServiceImpl implements FeatureEntitlementService {
  ...
  public void updateEntitlements(List<TenantFeatureUpdateItem> items) {
    ...
    Long operatorTenantId =
        TenantContextHolder.getActorTenantId() == null
            ? tenantId
    ...
    for (TenantFeaturePermission permission :
        tenantFeaturePermissionMapper.selectByTenantId(tenantId)) {
    ...
    for (TenantFeatureUpdateItem item : items) {
      ...
      if (existing == null) {
        ...
        writeAudit(
            tenantId,
            created.getObjectId(),
            status == STATUS_ENABLED ? "GRANT" : "REVOKE",
            null,
            status,
            null,
            permission.getId(),
    ...
    log.info(
        "Updated tenant feature entitlements: tenantId={}, operatorUserId={}, size={}",
        tenantId,
        operatorUserId,
        items.size());
  }
  ...
  private Permission resolvePermission(
    ...
    if (item.getPermissionId() != null) {
      ...
      if (permission == null) {
        throw new com.sunny.datapillar.common.exception.BadRequestException(
            "Parameter error", String.valueOf(item.getPermissionId()));
      }
      return permission;
    }
    ...
    if (permission == null) {
      throw new com.sunny.datapillar.common.exception.BadRequestException(
          "Parameter error", normalizedCode);
    }
    return permission;
  }

  private String resolveAction(
      boolean statusChanged, boolean permissionChanged, Integer beforeStatus, Integer afterStatus) {
    if (statusChanged) {
      if (beforeStatus != null
          && beforeStatus == STATUS_ENABLED
          && afterStatus == STATUS_DISABLED) {
        return "DISABLE";
      }
      if (beforeStatus != null
          && beforeStatus == STATUS_DISABLED
          && afterStatus == STATUS_ENABLED) {
        return "ENABLE";
      }
    }
