...
public class TenantRoleAdminController {
    ...
    @PostMapping
    @PreAuthorize("hasAuthority('ADMIN')")
    public ApiResponse<Void> create(@Valid @RequestBody RoleDto.Create dto) {
    ...
    @PreAuthorize("hasAuthority('ADMIN')")
    public ApiResponse<List<FeatureObjectDto.ObjectPermission>> permissions(@PathVariable Long roleId,
    ...
    @PreAuthorize("hasAuthority('ADMIN')")
    public ApiResponse<RoleDto.MembersResponse> members(@PathVariable Long roleId,
