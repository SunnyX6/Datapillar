...
/** Governance metadata proxy controller. */
@Tag(name = "GovernanceMetadata", description = "Governance metadata proxy interface")
@RestController
@RequestMapping("/biz/governance/metadata")
@RequiredArgsConstructor
public class GovernanceMetadataBizController {

  private static final String BASE_PATH = "/biz/governance/metadata";

  private final GovernanceMetadataService governanceMetadataService;

  @Operation(summary = "Proxy governance metadata request")
  @RequestMapping(
      value = {"", "/", "/**"},
      method = {
        RequestMethod.GET,
  ...
  public JsonNode proxy(HttpServletRequest request, @RequestBody(required = false) JsonNode body) {
    return governanceMetadataService.proxy(
