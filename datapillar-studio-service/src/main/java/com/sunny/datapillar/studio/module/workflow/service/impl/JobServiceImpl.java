...
/**
 * Task service implementation Implement task business process and rule verification
 *
 * @author Sunny
 * @date 2026-01-01
 */
...
public class JobServiceImpl implements JobService {
  ...
  public JobResponse getJobDetail(Long workflowId, Long id) {
    ...
    if (job == null) {
      throw new com.sunny.datapillar.common.exception.NotFoundException(
          "Task does not exist: workflowId=%s, jobId=%s", workflowId, id);
    }
    return job;
  }
  ...
  public Long createJob(Long workflowId, JobCreateRequest dto) {
    ...
    if (component == null) {
      throw new com.sunny.datapillar.common.exception.NotFoundException(
          "Task type does not exist: jobType=%s", dto.getJobType());
    }
    ...
    log.info(
        "Created job: id={}, workflowId={}, name={}",
        jobInfo.getId(),
        workflowId,
        jobInfo.getJobName());
  ...
  private JobInfo getWorkflowJobOrThrow(Long workflowId, Long jobId) {
    ...
    if (jobInfo == null || !workflowId.equals(jobInfo.getWorkflowId())) {
      throw new com.sunny.datapillar.common.exception.NotFoundException(
          "Task does not exist: workflowId=%s, jobId=%s", workflowId, jobId);
    }
    return jobInfo;
  }
  ...
  public void updateJobPositions(Long workflowId, JobLayoutSaveRequest dto) {
    ...
    log.info(
        "Updated job positions: workflowId={}, count={}", workflowId, dto.getPositions().size());
  }
}
