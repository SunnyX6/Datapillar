...
/**
 * FunctionObjectService implementation Implement functionObjectBusiness process and rule
 * verification
 *
 * @author Sunny
 * @date 2026-01-01
...
public class FeatureObjectServiceImpl implements FeatureObjectService {
  ...
  public List<FeatureTreeNodeItem> getFeatureObjectsByUserId(Long userId, String location) {
    ...
    List<FeatureObject> featureObjects =
        featureObjectMapper.findByUserId(tenantId, userId, location);
  ...
  @Override
  public List<FeatureTreeNodeItem> buildFeatureObjectTree(
      List<FeatureTreeNodeItem> featureObjects) {
    if (featureObjects == null || featureObjects.isEmpty()) {
      return new ArrayList<>();
    }

    Map<Long, List<FeatureTreeNodeItem>> featureObjectMap =
        featureObjects.stream()
            .collect(
  ...
  private void buildChildren(
      List<FeatureTreeNodeItem> parentNodes,
      Map<Long, List<FeatureTreeNodeItem>> featureObjectMap) {
    for (FeatureTreeNodeItem parentNode : parentNodes) {
      List<FeatureTreeNodeItem> children =
          featureObjectMap.getOrDefault(parentNode.getId(), new ArrayList<>());
      if (!children.isEmpty()) {
        children.sort(
            (n1, n2) -> {
              int sort1 = n1.getSort() != null ? n1.getSort() : 0;
  ...
  private List<FeatureTreeNodeItem> convertToFeatureObjectNodes(
      List<FeatureObject> featureObjects) {
